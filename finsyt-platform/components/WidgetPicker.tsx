...
export default function WidgetPicker() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,22,40,0.6)', zIndex: 300, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={closePicker}>
      <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 640, maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        {/* Header */}
        ...
        <div style={{ padding: '12px 20px 8px' }}>
          <div style={{ position: 'relative' }}>
            <svg style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#B0BCD0' }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search widgets..." style={{ width: '100%', padding: '8px 12px 8px 32px', background: '#F7F9FC', border: '1.5px solid #E8EDF4', borderRadius: 9, fontSize: 13, fontFamily: 'inherit', outline: 'none', color: '#0A1628', boxSizing: 'border-box' }} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, padding: '0 20px 12px', overflowX: 'auto' }}>
          {cats.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{ padding: '4px 12px', borderRadius: 999, fontSize: 11, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: 'inherit', background: cat === c ? '#1B4FFF' : '#F0F4FA', color: cat === c ? '#fff' : '#7D8FA9', whiteSpace: 'nowrap', flexShrink: 0 }}>{c}</button>
          ))}
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {filtered.map(w => (
              <button key={w.id} onClick={() => { addWidget(pickerPage, w.id); closePicker() }}
                style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 14px', background: '#F7F9FC', border: '1.5px solid #E8EDF4', borderRadius: 10, cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all 0.12s' }}
