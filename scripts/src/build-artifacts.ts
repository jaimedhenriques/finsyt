...
const HASH_EXCLUDE_DIRS = new Set([
  ...
  "build",
  ".next",
  // Next.js dev server output. The platform's next.config.ts sets
  // `distDir: '.next-dev'` when NODE_ENV !== 'production', so the dev server
  // and the production build never collide. Treat it like `.next/` for
  // hashing and output detection so dev-time recompiles don't churn the
  // build cache.
  ".next-dev",
  ".vite",
  ".turbo",
