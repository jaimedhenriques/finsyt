...
export default function Security() {
  useEffect(() => {
    ...
    document.title =
      "Security & Data Protection — Finsyt";

    const ensureMeta = (
      selector: string,
      attrs: Record<string, string>,
    ): { el: HTMLMetaElement | HTMLLinkElement; created: boolean } => {
    ...
    const description =
      "How Finsyt protects your data: encryption, tenant isolation, no model training on customer content, SSO, audit logs, subprocessors, and compliance roadmap.";

    const created: (HTMLMetaElement | HTMLLinkElement)[] = [];
    const previousValues: { el: Element; name: string; value: string | null }[] = [];

    const setMeta = (selector: string, attrs: Record<string, string>) => {
      const existing = document.head.querySelector(selector);
      if (existing) {
        Object.entries(attrs).forEach(([k, v]) => {
          previousValues.push({ el: existing, name: k, value: existing.getAttribute(k) });
    ...
    setMeta('meta[property="og:title"]', {
      property: "og:title",
      content: "Security & Data Protection — Finsyt",
    });
    ...
    setMeta('link[rel="canonical"]', {
      rel: "canonical",
      href: `${window.location.origin}/security`,
    });

    return () => {
      document.title = previousTitle;
      created.forEach((el) => el.remove());
      previousValues.forEach(({ el, name, value }) => {
