...
export default clerkMiddleware(async (auth, req) => {
  // Webhook routes use their own HMAC-based auth (svix signatures).
  // They must be reachable without a Clerk session; pass straight to
  // the security-header layer so the response still gets hardened headers.
  ...
  const res = applySecurityHeaders(req);
  // /platform/ is a utility gateway, not a public destination. Tell crawlers
  // not to index it so they don't treat the permanent redirect as a canonical
  // public entry point.
