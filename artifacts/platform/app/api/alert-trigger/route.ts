...
const AlertSchema = z.object({
  ...
  type: z.enum(['price_above', 'price_below', 'pct_change', 'volume_spike', 'news', 'news_sentiment']),
...
function buildEmail(alert: AlertPayload): EmailContent {
  ...
  const reasons: Record<AlertPayload['type'], string> = {
    ...
    news_sentiment: `news sentiment / volume deviated ≥ ${threshold}σ from its baseline`,
  }
