...
import { parseFyEndMonth, knownFyEndMonth } from "@/lib/intl-fiscal"
...
export async function GET(req: NextRequest) {
  ...
  if (!metric && !metrics) {
    try {
      ...
      // Fiscal year end: prefer coverage DB → FMP profile field → income statement date
      const fyEndFromProfile = parseFyEndMonth(p?.fiscalYearEnd ?? p?.fiscalDateEnd)
      const fyEndMonth = knownFyEndMonth(symbol) ?? fyEndFromProfile ?? 12

      return NextResponse.json({
        symbol,
        period: fmpPeriod,
        asOf: is.date || r.date || null,
        // Fiscal calendar metadata — used by the UI to label periods correctly
        // for non-calendar fiscal years (e.g. Toyota FY ends March).
        fiscalYearEndMonth: fyEndMonth,
        reportedCurrency:   is.reportedCurrency || p?.currency || "USD",
        snapshot: {
          name:             p?.companyName,
          sector:           p?.sector,
          industry:         p?.industry,
