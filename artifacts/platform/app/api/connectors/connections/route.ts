...
export async function POST(req: NextRequest) {
  ...
  // For catalog connectors that ship a `validateOperation`, run it immediately
  // so the user gets a clear "credentials accepted" / "got 401" signal on the
  // very same POST instead of having to navigate to the test button.
  //
  // Premium connectors (FactSet, CapIQ, Refinitiv, Bloomberg, PitchBook):
  //   Credentials are verified against the live upstream BEFORE the connection
  //   is considered saved. If validation fails, the row is deleted and a 422
  //   is returned — the user must fix their key and retry rather than end up
  //   with a dangling "error" connection they might not notice.
  //
  // Non-premium catalog connectors:
  //   Kept either way (status → "active" or "error") so the user can fix
  //   the credentials without re-entering the connector metadata.
  let validation:
  ...
  if (input.definitionSlug && input.credentials && Object.keys(input.credentials).length > 0) {
    ...
    if (entry?.validateOperation) {
      ...
      if (!outcome.ok && entry.isPremium) {
        // Premium BYO connectors must pass validation before being saved.
        // Delete the row that was just inserted (credentials + operations
        // cascade via FK) and return a descriptive 422 so the user can
        // correct their institutional key and retry from the Connect modal.
        await withOrgContext(localOrgId, async (tx) => {
          await tx.delete(connectionsTable).where(eq(connectionsTable.id, created.id));
        });
        await auditLog({
          orgId: localOrgId,
          actorId: userId,
          actorType: "user",
          action: "connector.connection.validation_failed",
          resourceType: "connection",
          resourceId: created.id,
          metadata: {
            displayName: created.displayName,
            slug: input.definitionSlug,
        ...
        return NextResponse.json(
          {
            error: outcome.error || "Credential validation failed",
            detail: outcome.detail,
            validation: {
              ok: false,
              status: outcome.status,
