  ...
  try {
    if (strategy.kind === "mcp") {
      ...
      const outcome = await runValidationCall({
        ...
        // Forward any per-connector copy overrides + identity-path so the
        // Test button surfaces "Connected as <username>" / provider-specific
        // failure copy with the same parity as the inline create-time probe.
