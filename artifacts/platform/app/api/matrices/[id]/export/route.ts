...
import { getHouseStyle, houseStyleAuditSummary } from '@/lib/house-style'
...
export async function GET(req: NextRequest, { params }: RouteParams) {
  ...
  // House style + verification provenance: load the org's house style, run the
  // pure verification engine over the export, and record the deliverable's
  // review status + applied house style in the audit trail. The PPTX brand is
  // overlaid with the desk palette so exports come out in the firm's voice.
  ...
  try {
    await audit.log({
      ...
      metadata: {
        ...
        verification: { passed: report.passed, summary: report.summary },
        houseStyle: houseStyleAuditSummary(houseStyle),
      },
    })
  } catch {
    /* swallow — audit failure must not block the export */
  }
  ...
  const buf = await buildMatrixPptx(data, houseStyle.enabled ? houseStyle.config.brand : undefined)
