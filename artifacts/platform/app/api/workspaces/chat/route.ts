...
export async function POST(req: NextRequest) {
  ...
  // Org guard: when a workspace is referenced, verify it belongs to the
  // caller's active org before resolving source ids. This prevents a user
  // who switched orgs (or was removed from an org) from submitting a
  // workspaceId from the previous org and having the platform summarise the
  // old tenant's documents.
  if (workspaceId) {
    if (!orgId) {
    ...
    try {
      ...
      const belongs = await workspaceBelongsToOrg(localOrgId, workspaceId)
      if (!belongs) {
        return NextResponse.json({ error: "Forbidden" }, { status: 403 })
      }
    } catch {
