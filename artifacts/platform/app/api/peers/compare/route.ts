...
// margins, growth) plus three forward-looking metrics:
//   - forwardPe     REAL when the FMP estimates feed has a forward consensus
//                   EPS (price ÷ next-FY EPS); otherwise demo.
//   - evEbitdaNtm   REAL when the quote returns an absolute enterprise value
//                   (FMP profile) AND the estimates feed has a forward
//                   consensus EBITDA (EV ÷ fwd EBITDA); otherwise demo.
//   - optionsItmPct REAL when the FMP options chain is available (FMP_API_KEY
//                   configured, endpoint returns contracts with inTheMoney
//                   field); otherwise demo.
//
