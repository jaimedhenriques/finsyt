...
const INSIDER_KEYWORDS = /\b(insider|10b5-1|form 4|share (sale|purchase|buy)|stock sale|stock purchase|bought shares|sold shares|disposed of shares|filed form 4|insider trading)\b/i
...
export async function GET(req: NextRequest) {
  const gate = await requireFeature('insider')
  ...
  // ── Source 4: FinanceFlow news (insider-tagged headlines as alt-data context)
  // Surfaces newswire mentions of Form 4 / 10b5-1 filings the structured feeds
  // can lag on. Returned alongside the structured rows under `relatedNews`.
  let relatedNews: any[] = []
  if (PROVIDERS.financeflow) {
    try {
      const news = await financeflowNews(symbol, 30)
      relatedNews = (news || [])
        .filter((n: { title?: string; summary?: string }) =>
          INSIDER_KEYWORDS.test(`${n.title ?? ''} ${n.summary ?? ''}`))
        .slice(0, 10)
    } catch (e) { console.warn('[insider] FinanceFlow alt-data failed:', (e as Error).message) }
  }
  ...
  return NextResponse.json({
    ...
    relatedNews,
    sources: ['fmp', 'eodhd', 'finnhub', ...(PROVIDERS.financeflow ? ['financeflow'] : [])],
  })
}
