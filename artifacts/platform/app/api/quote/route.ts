...
async function resolveQuote(symbol: string): Promise<Record<string, any> | null> {
  ...
  // dedicated multi-asset waterfall. Equities fall through to the existing
  // provider chain below, stamped with assetClass:'equity'.
  const classified = classifySymbol(symbol)
  if (classified.assetClass !== 'equity') {
    const q = await resolveMultiAssetQuote(classified).catch(() => null)
    return q ? ({ ...q } as Record<string, any>) : null
  }
...
export async function GET(req: NextRequest) {
  ...
  if (symbolsParam) {
    ...
    const quotes = resolved
      ...
      .map(q => ({ assetClass: 'equity', ...q, asOf }))
  ...
  return NextResponse.json({ assetClass: 'equity', ...quote, asOf: new Date().toISOString() })
}
