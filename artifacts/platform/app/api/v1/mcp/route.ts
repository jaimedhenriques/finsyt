...
import { authenticateApiRequest, consumeExtraTokens, corsPreflight } from "@/lib/api-key-auth";
...
/** Maximum number of JSON-RPC requests allowed in a single batch call. */
const MCP_BATCH_LIMIT = 25;
...
export async function POST(req: NextRequest) {
  ...
  if (Array.isArray(body)) {
    if (body.length > MCP_BATCH_LIMIT) {
      const res = NextResponse.json(
        rpcError(null, -32600, `Batch size ${body.length} exceeds the maximum of ${MCP_BATCH_LIMIT} requests per call.`),
        { status: 400 },
      );
      res.headers.set("Access-Control-Allow-Origin", "*");
      return res;
    }
    // The first token was already consumed by authenticateApiRequest.
    // Consume one additional token for each extra item in the batch so that
    // batching cannot be used to bypass per-key rate limits.
    const extraItems = body.length - 1;
    if (extraItems > 0) {
      const extra = consumeExtraTokens(auth.key.id, auth.key.rateLimitPerMinute, extraItems);
      if (!extra.ok) {
        const retryAfter = Math.ceil(extra.resetMs / 1000);
        const res = NextResponse.json(
    ...
    const responses = (
      await runWithApiKeyEntitlement(auth.key, () =>
        Promise.all(body.map((r) => handleRpc(r))),
      )
    ).filter((r) => r !== null);
    return NextResponse.json(responses);
  }

  const result = await runWithApiKeyEntitlement(auth.key, () => handleRpc(body));
  if (result === null) {
