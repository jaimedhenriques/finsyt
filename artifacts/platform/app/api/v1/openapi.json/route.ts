...
function spec(origin: string) {
  return {
    ...
    paths: {
      ...
      "/rates/yield-curve": {
        get: {
          summary: "U.S. Treasury yield curve (1M–30Y) with curve spreads; pass ?date= for a historical snapshot",
          parameters: [
            { name: "date", in: "query", schema: { type: "string", format: "date", description: "Historical snapshot date (YYYY-MM-DD); omit for latest" } },
          ],
          responses: { "200": { description: "Yield curve points, spreads and per-tenor source attribution" } },
        },
      },
      "/rates/reference": {
        get: {
          summary: "Benchmark reference rates board (SOFR, EFFR, SONIA, €STR) with recent history",
          responses: { "200": { description: "Reference rate series with source attribution" } },
        },
      },
      "/rates/credit-spreads": {
        get: {
          summary: "Credit spreads (ICE BofA IG vs HY option-adjusted spreads) with IG–HY differential",
          parameters: [
            { name: "periods", in: "query", schema: { type: "integer", default: 365, description: "Lookback window in days" } },
          ],
          responses: { "200": { description: "IG / HY OAS series, differential and source attribution" } },
        },
      },
      "/agent/persona": {
        get: {
