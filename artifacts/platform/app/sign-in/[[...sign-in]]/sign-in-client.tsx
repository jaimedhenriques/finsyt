...
export function SignInClient({
  ...
  async function handleEnterpriseSSO(email: string) {
    if (!isLoaded || !signIn) return
    setErrorMessage(null)
    ...
    try {
      ...
      const msg =
        (err as { errors?: Array<{ message?: string; longMessage?: string }> })
          ?.errors?.[0]?.longMessage ??
        (err as { errors?: Array<{ message?: string }> })?.errors?.[0]?.message ??
        (err instanceof Error ? err.message : "Enterprise SSO sign-in failed.")
      // Surface a helpful hint when no enterprise connection is configured
      setErrorMessage(
        msg.includes("not supported") || msg.includes("no enterprise")
          ? `No enterprise SSO connection found for this email domain. Contact your IT admin or Finsyt support to configure a SAML connection. (${msg})`
          : msg,
      )
    } finally {
  ...
  return (
    <SignInPage
      ...
      enterpriseSSO={{ onSubmit: handleEnterpriseSSO, submitting: ssoSubmitting }}
      demoSignIn={
        demoEnabled
          ? {
              email: demoEmail,
