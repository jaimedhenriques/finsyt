    ...
    <div className="page-content" style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {isConnected ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", background: 'var(--pos-dim)', border: "1px solid #A7F3D0", borderRadius: 8 }}>
              {user?.img_url && <img src={user.img_url} alt="" style={{ width: 22, height: 22, borderRadius: "50%" }} />}
              <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--pos)' }}>{user?.handle || user?.email || "Connected"}</span>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <div style={{ position: "relative", flex: 1 }}>
          <svg style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)", pointerEvents: "none" }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" /></svg>
          <input
            className="input"
            value={fileKey}
      ...
      {error && (
        <div style={{ padding: "10px 14px", background: 'var(--neg-dim)', border: "1px solid #FECACA", borderRadius: 10, color: 'var(--neg)', fontSize: 13, fontWeight: 600, marginBottom: 16 }}>
          ⚠️ {error}
        </div>
      )}
      ...
      {activeTab === "file" && fileData && (
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
            ...
            <div style={{ flex: 1 }}>
              <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--bg-elevated)', marginBottom: 4 }}>{fileData.name}</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12 }}>
            {components.map(comp => (
              <div key={comp.key} className="card" style={{ padding: 14, display: "flex", gap: 12, alignItems: "flex-start", transition: "border-color 0.15s", cursor: "default" }}
                ...
                {comp.thumbnailUrl ? (
                  <img src={comp.thumbnailUrl} alt={comp.name} style={{ width: 48, height: 48, objectFit: "contain", borderRadius: 6, background: 'var(--bg-page)', border: "1px solid #E8EDF4", flexShrink: 0 }} />
                ) : (
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--bg-elevated)', marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{comp.name}</div>
                  {comp.description && (
                  ...
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                    ...
                    {comp.updatedAt && <span style={{ fontSize: 10, color: "var(--text-muted)" }}>{new Date(comp.updatedAt).toLocaleDateString()}</span>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="card" style={{ marginTop: 24, padding: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, fontSize: 12 }}>
          {[
            ...
            <div key={ep.path} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", background: 'var(--bg-page)', borderRadius: 6 }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: 'var(--pos)', fontFamily: "monospace" }}>GET</span>
