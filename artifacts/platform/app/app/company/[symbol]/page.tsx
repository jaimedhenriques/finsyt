...
import EsgCard from '@/components/company/EsgCard'
...
import FixedIncomeTab from '@/components/company/FixedIncomeTab'
...
import IntelligenceTiles from '@/components/intelligence/IntelligenceTiles'
...
// ── Types ────────────────────────────────────────────────────────────────────
type CompanyTab = 'overview' | 'financials' | 'transcripts' | 'filings' | 'estimates' | 'valuations' | 'fixed-income' | 'technicals' | 'options' | 'ownership' | 'relationships' | 'news' | 'questions' | 'ai-analysis' | 'notes' | 'data' | 'fund'
...
/** Currency symbol for display — maps common ISO codes to glyphs, falls back to the code. */
function currSym(c?: string): string {
  if (!c) return '$'
  const MAP: Record<string, string> = { USD: '$', EUR: '€', GBP: '£', JPY: '¥', CHF: 'Fr', AUD: 'A$', CAD: 'C$', CNY: '¥', HKD: 'HK$', KRW: '₩', SEK: 'kr', NOK: 'kr', DKK: 'kr', SGD: 'S$', INR: '₹', BRL: 'R$', MXN: 'MX$', TWD: 'NT$' }
...
function fmtLarge(n: any, currency?: string) {
  if (!n) return '—'; const v = Number(n); const s = currSym(currency)
...
function fmtB(n: any, currency?: string) {
  if (!n) return '—'; const v = Number(n); const s = currSym(currency)
  if (v >= 1e9) return s + (v / 1e9).toFixed(1) + 'B'
  if (v >= 1e6) return s + (v / 1e6).toFixed(0) + 'M'
  return s + v.toLocaleString()
...
// ── International coverage badge ─────────────────────────────────────────────
// Shows a small pill indicating the data-coverage level for non-US symbols.
// Only rendered when the symbol is in the verified seed basket.
const INTL_COVERAGE_DB_SYMBOLS: Set<string> = new Set([
  'ASML.AS','MC.PA','SAP.DE','NESN.SW','NOVN.SW','SHELL.AS','TTE.PA',
  'HSBA.L','AZN.L','GSK.L','BP.L','BARC.L','ULVR.L','RIO.L','LLOY.L',
  'VOW3.DE','BMW.DE','SIE.DE','BAS.DE','OR.PA','BNP.PA','ENEL.MI','ENI.MI',
  'INGA.AS','PHIA.AS',
  '7203.T','9984.T','6758.T','8306.T','7267.T','6861.T','4519.T','8035.T',
  '6501.T','9432.T','6954.T','7751.T','4543.T','6098.T',
])
const INTL_PARTIAL_SYMBOLS: Set<string> = new Set([
  'ENEL.MI','ENI.MI','4519.T','7751.T','4543.T',
])

function IntlCoverageBadge({ symbol }: { symbol: string }) {
  if (!INTL_COVERAGE_DB_SYMBOLS.has(symbol)) return null
  const isPartial = INTL_PARTIAL_SYMBOLS.has(symbol)
  const label = isPartial ? 'Partial coverage' : 'Full coverage'
  const style = isPartial
    ? { background: 'rgba(245,158,11,0.14)', color: 'rgb(217,119,6)', border: '1px solid rgba(245,158,11,0.3)' }
    : { background: 'rgba(16,185,129,0.12)', color: 'rgb(5,150,105)', border: '1px solid rgba(16,185,129,0.3)' }
  return (
    <span title={`International data ${label.toLowerCase()} via Finsyt provider waterfall`}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 7px', borderRadius: 999, fontSize: 10, fontWeight: 700, letterSpacing: '0.04em', ...style }}>
      🌐 {label}
    </span>
  )
}
...
export default function CompanyPage({ params }: { params: Promise<{ symbol: string }> }) {
  ...
  useEffect(() => {
    ...
    const t = sp.get('tab')
    const allowed = ['overview','financials','transcripts','filings','estimates','valuations','technicals','options','ownership','relationships','news','questions','ai-analysis','notes','data','fund'] as const
  ...
  const quoteCurrency = quote?.currency && quote.currency !== 'USD' ? quote.currency : undefined
  const isNonUSD = !!quoteCurrency
  const KEY_METRICS = [
    { l: 'Market Cap',    v: fmtLarge(quote?.marketCap, quote?.currency) },
    { l: 'P/E Ratio',     v: quote?.pe > 0 ? `${fmt(quote.pe, 1)}x` : '—' },
    { l: 'Revenue (TTM)', v: fmtLarge(quote?.revenue, quote?.currency) },
    { l: 'EPS (TTM)',     v: quote?.eps ? `${currSym(quote?.currency)}${fmt(quote.eps)}` : '—' },
    { l: '52W High',      v: quote?.high52w ? `${currSym(quote?.currency)}${fmt(quote.high52w)}` : '—' },
    { l: '52W Low',       v: quote?.low52w  ? `${currSym(quote?.currency)}${fmt(quote.low52w)}`  : '—' },
    ...
    { l: 'Float',         v: quote?.sharesFloat ? fmtLarge(quote.sharesFloat, quote?.currency) : '—' },
    ...
    <div style={{ padding: '1.75rem', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 4 }}>
            ...
            {isNonUSD && (
              <span title={`Reported in ${quote?.currency}`}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 999, fontSize: 10, fontWeight: 800, background: 'rgba(99,102,241,0.12)', color: 'rgb(99,102,241)', border: '1px solid rgba(99,102,241,0.25)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {quote?.currency}
              </span>
            )}
            <IntlCoverageBadge symbol={SYM} />
            <LiveEventBadge symbol={SYM} />
          </div>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
            {quote?.industry || ''}
            {quote?.country && quote.country !== 'US' && (
              <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--text-muted)' }}>· {quote.country}</span>
            )}
          </p>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: '2.25rem', fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.03em', lineHeight: 1 }}>{currSym(quote?.currency)}{fmt(quote?.price)}</div>
          ...
          <div style={{ marginTop: 10, display: 'flex', justifyContent: 'flex-end', gap: 8, flexWrap: 'wrap' }}>
            ...
            <ExportWordButton symbol={SYM} />
            <ExportEmlButton symbol={SYM} />
            <ExportToDeckButton symbol={SYM} />
            <DelegateButton
              variant="ghost"
              label="Delegate to agent"
              context={{ surface: 'company', symbol: SYM, label: quote?.name || SYM, defaultDeliverable: 'memo' }}
            />
          </div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        <Tabs
          ...
          items={[
            ...
            { id: 'fixed-income',label: 'Fixed Income', badge: <Badge tone="violet" style={{ fontSize: 9 }}>NEW</Badge> },
            ...
            { id: 'fund',        label: 'Fund / ETF', badge: <Badge tone="violet" style={{ fontSize: 9 }}>NEW</Badge> },
            { id: 'news',        label: 'News' },
      ...
      {tab === 'overview' && (
        <div key="t-overview" className="fade-up">
          ...
          {/* ESG scores (Yahoo, supplementary) */}
          <div style={{ marginBottom: 20 }}><EsgCard symbol={SYM} /></div>
          {/* Price chart */}
          ...
          {/* ── Global Intelligence tiles (Task #614): geopolitical risk,
              sanctions screening, cyber threats, and news intelligence brief.
              All backed by public free-to-use APIs — no connection required. */}
          <IntelligenceTiles
            symbol={SYM}
            companyName={quote?.name || SYM}
            hqCountryIso={quote?.country || 'US'}
            sector={quote?.sector}
          />
      ...
      {/* ── FIXED INCOME — issuer credit & debt ── */}
      {tab === 'fixed-income' && (
        <div key="t-fixed-income" className="fade-up">
          <FixedIncomeTab symbol={SYM} />
        </div>
      )}
      ...
      {/* ── FUND / ETF — profile, holdings, weightings (Yahoo) ── */}
      {tab === 'fund' && (
        <div key="t-fund" className="fade-up">
...
// ── Export Word Memo button ──────────────────────────────────────────────────
// Calls /api/copilot/word and downloads a branded .docx investment memo.
function ExportWordButton({ symbol }: { symbol: string }) {
  const [busy, setBusy] = useState(false)
  const [status, setStatus] = useState<string | null>(null)

  async function onExport() {
    setBusy(true); setStatus('Building Word memo…')
    try {
      const r = await fetch(`${BASE}/api/copilot/word`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const a    = document.createElement('a')
      a.href = url
      a.download = `${symbol} Investment Memo.docx`
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
      <button
        onClick={onExport}
        disabled={busy}
        title={`Download ${symbol} investment memo as Word document (.docx)`}
        style={{
      ...
      >
        ...
        {busy ? 'Building…' : 'Word memo'}
      </button>
      {status && (
        <span style={{ fontSize: 10, color: status.startsWith('Failed') || status.startsWith('Export') ? 'var(--red)' : 'var(--text-muted)' }}>
          {status}
        </span>
      )}
    </div>
...
// ── Export Email Draft button ────────────────────────────────────────────────
// Calls /api/copilot/eml and downloads a pre-populated Outlook .eml draft.
function ExportEmlButton({ symbol }: { symbol: string }) {
  const [busy, setBusy]     = useState(false)
  const [status, setStatus] = useState<string | null>(null)

  async function onExport() {
    setBusy(true); setStatus('Building email…')
    try {
      const r = await fetch(`${BASE}/api/copilot/eml`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // Ticker-only mode: the server assembles live memo data, extracts real
        // key points, metrics, and citations, and generates a rich research
        // summary email — no canned/placeholder text.
        body: JSON.stringify({ ticker: symbol }),
      })
      if (!r.ok) {
        const j = await r.json().catch(() => ({}))
      ...
      const a    = document.createElement('a')
      a.href = url
      a.download = `${symbol} Research Summary.eml`
      document.body.appendChild(a); a.click(); a.remove()
      URL.revokeObjectURL(url)
      setStatus('Downloaded ✓')
      setTimeout(() => setStatus(null), 4000)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
      <button
        onClick={onExport}
        disabled={busy}
        title={`Download Outlook email draft for ${symbol} (.eml)`}
        style={{
          fontSize: 12, fontWeight: 600, padding: '7px 11px', borderRadius: 8,
          border: '1px solid var(--border)', color: 'var(--text-secondary)',
          background: 'transparent', cursor: busy ? 'wait' : 'pointer',
