...
// ── Types ────────────────────────────────────────────────────────────────────
type CompanyTab = 'overview' | 'financials' | 'transcripts' | 'filings' | 'estimates' | 'valuations' | 'ownership' | 'news' | 'questions' | 'ai-analysis' | 'notes' | 'data'
...
export default function CompanyPage({ params }: { params: Promise<{ symbol: string }> }) {
  ...
  useEffect(() => {
    ...
    const t = sp.get('tab')
    const allowed = ['overview','financials','transcripts','filings','estimates','valuations','ownership','news','questions','ai-analysis','notes','data'] as const
      ...
      {tab === 'overview' && (
        <div key="t-overview" className="fade-up">
          ...
          <AltDataTiles
            ...
            companyName={quote?.name || SYM}
            insiders={insiders}
            onCite={(label, body) => setCitation({ open: true, label, body })}
          />
        </div>
      )}
