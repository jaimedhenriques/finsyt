...
import {
  PageHero, MetricTile, LoadingTile, ContextualAskBar,
...
/* ──────────────────────────────────────────────────────────────────────────
   Overview is now 100% data-driven. Anything that used to be hardcoded
   (METRICS, COVERAGE, NEWS, TRENDING_TOPICS, DATA_SOURCES, mulberry32 spark
   generator) has been removed. Indices come from /api/quote, headlines from
   /api/news, coverage from /api/watchlist + /api/quote, sparklines from the
   quote payload (which providers populate inline).
   ──────────────────────────────────────────────────────────────────────── */
...
function EmptyCoverage() {
  return (
    <div style={{
      padding: '28px 22px', borderRadius: 12, border: '1px dashed var(--border)',
      background: 'var(--bg-card)', display: 'flex', alignItems: 'center', gap: 14,
    }}>
      <div style={{
        width: 40, height: 40, borderRadius: 10, background: 'var(--accent-dim)',
        color: 'var(--accent-text)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
      }}>
        <NAV_ICONS.watchlist width={20} height={20} strokeWidth={ICON_STROKE} />
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>Your watchlist is empty</div>
        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
          Add tickers and Finsyt will surface filings, prices, and agent runs across them here.
        </div>
      </div>
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: '18px 16px', fontSize: 12, color: 'var(--text-secondary)' }}>
          No tickers yet. <Link href="/app/watchlist" style={{ color: 'var(--accent-text)' }}>Add some</Link>.
        </div>
      ) : coverage.map((r, i) => {
    ...
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 18 }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: '12px 0', fontSize: 13, color: 'var(--text-secondary)' }}>
          No fresh headlines right now. Finsyt will refresh shortly.
        </div>
      ) : news.map((n, i) => {
    ...
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 18 }}>
      ...
      {loading ? (
        ...
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', padding: '8px 0' }}>
          No upcoming earnings on your watchlist in the next two weeks.
        </div>
      ) : items.map((e, i) => (
