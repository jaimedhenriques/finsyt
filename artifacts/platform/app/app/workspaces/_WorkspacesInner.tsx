...
export default function WorkspacesInner() {
  ...
  const addSource = useCallback(async (type: Source["type"], data: { name: string; url?: string; text?: string; file?: File; size?: string }) => {
    ...
    const newSource: Source = { id: localId, name: data.name, type, status: "processing", size: data.size, addedAt: new Date().toISOString() }
    ...
    try {
      ...
      setWorkspace(w => ({
        ...
        sources: w.sources.map(s => s.id === localId
          ? { ...s, id: serverId, status: result.success ? "ready" : "error", chunkCount: result.chunkCount, size: result.size || data.size }
          : s
        ),
      }))
