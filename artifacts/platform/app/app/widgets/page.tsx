...
const COLORS = ['var(--accent)','var(--pos)','var(--amber)','var(--neg)','#8B5CF6','#0891B2','#EC4899']
const SOURCE_BADGE: Record<string,string> = { Finnhub:'#0891B2', FMP:'var(--pos)', FRED:'var(--neg)', AV:'#8B5CF6' }
...
function ChartPreview({ type, color, label }: { type: ChartType; color: string; label: string }) {
  ...
  if (type === 'metric') return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:4 }}>
      ...
      <div style={{ fontSize:12, fontWeight:600, color:'var(--pos)' }}>▲ +2.57% today</div>
  ...
  if (type === 'table') return (
    <div style={{ fontSize:11, padding:'8px 0' }}>
      {['Q1 \'26','Q4 \'25','Q3 \'25','Q2 \'25'].map((q,i)=>(
        <div key={q} style={{ display:'flex', gap:12, padding:'5px 8px', background:i%2===0?'var(--bg-page)':'transparent', borderRadius:4 }}>
          <span style={{ flex:1, color:'var(--text-secondary)' }}>{q}</span>
          <span style={{ fontWeight:700, color:'var(--text-primary)' }}>${(39.3-i*2).toFixed(1)}B</span>
          <span style={{ color:'var(--pos)', fontWeight:600 }}>+{(73-i*8).toFixed(0)}%</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      {type==='bar'
        ? <BarChart {...common}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="t" tick={{fontSize:9,fill:'var(--text-muted)'}} /><YAxis hide /><Tooltip contentStyle={{fontSize:11}} /><Bar dataKey="v" fill={color} radius={[3,3,0,0]} /></BarChart>
        : type==='line'
        ? <LineChart {...common}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="t" tick={{fontSize:9,fill:'var(--text-muted)'}} /><YAxis hide /><Tooltip contentStyle={{fontSize:11}} /><Line type="monotone" dataKey="v" stroke={color} strokeWidth={2} dot={false} /></LineChart>
        : <AreaChart {...common}><defs><linearGradient id="cg" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={color} stopOpacity={0.2}/><stop offset="95%" stopColor={color} stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="t" tick={{fontSize:9,fill:'var(--text-muted)'}} /><YAxis hide /><Tooltip contentStyle={{fontSize:11}} /><Area type="monotone" dataKey="v" stroke={color} strokeWidth={2} fill="url(#cg)" dot={false}/></AreaChart>
      }
    </ResponsiveContainer>
  )
}
...
function SavedWidgetCard({ w, onDelete }: { w: WidgetConfig; onDelete: () => void }) {
  ...
  return (
    <div style={{ background:'var(--bg-card)', border:'1.5px solid var(--border)', borderRadius:12, overflow:'hidden', transition:'box-shadow 0.15s' }}
...
export default function WidgetsPage() {
  ...
  return (
    <div style={{ padding:'1.25rem 1.5rem', background:'var(--bg-page)', minHeight:'calc(100vh - 60px)' }}>
      {/* Header */}
      <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:20 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => setTab('library')} style={{ padding:'7px 16px', background:tab==='library'?'var(--bg-elevated)':'#fff', color:tab==='library'?'#fff':'#3D4F6E', border:'1.5px solid', borderColor:tab==='library'?'var(--bg-elevated)':'var(--border)', borderRadius:8, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>My Widgets ({saved.length})</button>
          <button onClick={() => setTab('builder')} style={{ padding:'7px 16px', background:tab==='builder'?'var(--accent)':'#fff', color:tab==='builder'?'#fff':'#3D4F6E', border:'1.5px solid', borderColor:tab==='builder'?'var(--accent)':'var(--border)', borderRadius:8, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>+ New Widget</button>
        </div>
      </div>

      <ContextualAskBar
        context="Widget Builder"
        contextData={{ page: 'widgets', tab, savedCount: saved.length }}
        chips={[
          { label: 'Suggest widgets',   prompt: 'Suggest 4 custom widgets I should build to monitor my watchlist effectively.' },
          { label: 'Margin tracker',    prompt: 'Design a widget that tracks gross, operating, and net margin trends across a peer group I name.' },
          { label: 'Capex / FCF view',  prompt: 'Build a widget spec showing capex intensity and free cash flow conversion for a sector I select.' },
          { label: 'Estimate revisions',prompt: 'Mock up a widget that highlights consensus estimate revisions over the last 30/60/90 days.' },
        ]}
        placeholder="Ask Finsyt to design a widget…"
      ...
      {tab === 'library' && (
        <div>
          {saved.length === 0 ? (
            <div style={{ textAlign:'center', padding:'60px 24px', background:'var(--bg-card)', border:'2px dashed var(--border)', borderRadius:14 }}>
              <div style={{ fontSize:36, marginBottom:12 }}>🧩</div>
              <div style={{ fontSize:15, fontWeight:700, color:'var(--text-secondary)', marginBottom:6 }}>No custom widgets yet</div>
              <p style={{ fontSize:13, color:'var(--text-muted)', marginBottom:20 }}>Build your first widget — choose data points, chart type, and companies.</p>
              <button onClick={() => setTab('builder')} style={{ padding:'9px 22px', background:'var(--accent)', color:'#fff', border:'none', borderRadius:9, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>+ Build a Widget</button>
            </div>
          ) : (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:16 }}>
              ...
              <button onClick={() => setTab('builder')} style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8, background:'transparent', border:'2px dashed #D0DAE8', borderRadius:12, cursor:'pointer', fontFamily:'inherit', minHeight:200, transition:'border-color 0.15s' }}
                onMouseEnter={e=>(e.currentTarget.style.borderColor='var(--accent)')} onMouseLeave={e=>(e.currentTarget.style.borderColor='#D0DAE8')}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:16 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div style={{ background: '#fff', border:'1px solid #E8EDF4', borderRadius:12, padding:'16px 18px' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-secondary)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10 }}>1 · Companies / Tickers</div>
              <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:8 }}>
                {symbols.map(s => (
                  <span key={s} style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', background:'rgba(27,79,255,0.12)', borderRadius:999, fontSize:12, fontWeight:700, color:'var(--accent)' }}>
                    {s}
                ...
                <div style={{ display:'flex', gap:5 }}>
                  <input value={symbolInput} onChange={e=>setSymInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addSymbol()} placeholder="Add ticker…" style={{ border:'1px solid #E8EDF4', borderRadius:7, padding:'4px 9px', fontSize:12, fontFamily:'inherit', outline:'none', width:110, color:'var(--text-primary)' }} />
                  <button onClick={addSymbol} style={{ padding:'4px 10px', background:'var(--accent)', color: '#fff', border:'none', borderRadius:7, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>+</button>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#fff', border:'1px solid #E8EDF4', borderRadius:12, padding:'16px 18px' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-secondary)', letterSpacing:'0.08em', textTransform:'uppercase', flex:1 }}>2 · Data Points</div>
                {selected.length > 0 && <span style={{ fontSize:11, padding:'2px 8px', borderRadius:999, background:'rgba(27,79,255,0.12)', color:'var(--accent)', fontWeight:700 }}>{selected.length} selected</span>}
              </div>
                ...
                {filtered.map(d => {
                  ...
                  return (
                    <button key={d.id} onClick={() => toggleDataPoint(d.id)}
                      style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 11px', background:sel?'var(--accent-dim)':'var(--bg-page)', border:`1.5px solid ${sel?'var(--accent)':'var(--border)'}`, borderRadius:9, cursor:'pointer', textAlign:'left', fontFamily:'inherit', transition:'all 0.1s' }}>
                      <div style={{ width:18, height:18, borderRadius:4, border:`2px solid ${sel?'var(--accent)':'#D0DAE8'}`, background:sel?'var(--accent)':'transparent', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all 0.1s' }}>
                        {sel && <svg width="10" height="10" viewBox="0 0 12 12" fill="none" stroke='var(--bg-card)' strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="2 6 5 9 10 3"/></svg>}
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ background: '#fff', border:'1px solid #E8EDF4', borderRadius:12, padding:'14px 18px' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-secondary)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10 }}>3 · Time Frequency</div>
              <div style={{ display:'flex', gap:6 }}>
                {(['daily','weekly','monthly','quarterly','annual'] as DataFreq[]).map(f => (
                  <button key={f} onClick={() => setFreq(f)} style={{ padding:'5px 12px', borderRadius:7, border:'1.5px solid', borderColor:freq===f?'var(--accent)':'var(--border)', background:freq===f?'var(--accent-dim)':'var(--bg-page)', color:freq===f?'var(--accent)':'#7D8FA9', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit', textTransform:'capitalize' }}>{f}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div style={{ background: '#fff', border:'1px solid #E8EDF4', borderRadius:12, padding:'14px 16px' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-secondary)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10 }}>Chart Type</div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:6 }}>
                {CHART_TYPES.map(c => (
                  <button key={c.id} onClick={() => setChartType(c.id)} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4, padding:'8px 4px', borderRadius:8, border:'1.5px solid', borderColor:chartType===c.id?'var(--accent)':'var(--border)', background:chartType===c.id?'var(--accent-dim)':'var(--bg-page)', cursor:'pointer', fontFamily:'inherit', transition:'all 0.1s' }}>
                    <span style={{ fontSize:16 }}>{c.icon}</span>
                    <span style={{ fontSize:9.5, fontWeight:600, color:chartType===c.id?'var(--accent)':'#7D8FA9' }}>{c.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Color */}
            <div style={{ background: '#fff', border:'1px solid #E8EDF4', borderRadius:12, padding:'14px 16px' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-secondary)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10 }}>Colour</div>
              <div style={{ display:'flex', gap:8 }}>
                {COLORS.map(c => (
                  <button key={c} onClick={() => setColor(c)} style={{ width:26, height:26, borderRadius:'50%', background:c, border:`3px solid ${color===c?'var(--bg-elevated)':'transparent'}`, cursor:'pointer', transition:'border-color 0.1s' }} />
                ))}
              </div>
            </div>
            ...
            {/* Preview */}
            <div style={{ background: '#fff', border:'1px solid #E8EDF4', borderRadius:12, padding:'14px 16px' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-secondary)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10 }}>Preview</div>
              <div style={{ height:160, background:'var(--bg-page)', borderRadius:8, padding:'8px', display:'flex', alignItems:'stretch' }}>
                {selected.length === 0
            ...
            {/* Widget name */}
            <div style={{ background: '#fff', border:'1px solid #E8EDF4', borderRadius:12, padding:'14px 16px' }}>
              <div style={{ fontSize:11, fontWeight:700, color:'var(--text-secondary)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:8 }}>Widget Name</div>
              <input value={widgetName} onChange={e=>setWidgetName(e.target.value)} placeholder="e.g. NVDA Revenue + Margin Trend" style={{ width:'100%', padding:'8px 10px', border:'1.5px solid #E8EDF4', borderRadius:8, fontSize:13, fontFamily:'inherit', outline:'none', boxSizing:'border-box', color:'var(--text-primary)' }}
            ...
            <button onClick={saveWidget} disabled={selected.length===0}
              style={{ padding:'12px 20px', background:selected.length===0?'var(--border)':saved_?'var(--pos)':'var(--accent)', color:selected.length===0?'var(--text-muted)':'#fff', border:'none', borderRadius:10, fontSize:14, fontWeight:700, cursor:selected.length===0?'default':'pointer', fontFamily:'inherit', transition:'background 0.2s' }}>
