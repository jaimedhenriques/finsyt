...
interface Article { title:string; source:string; publishedAt:string; url:string; summary?:string; tickers?:string[]; theme?:string; sentimentLabel?:'positive'|'neutral'|'negative'; sentimentScore?:number }
            ...
            <Card key={i} padding="16px 18px">
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8,fontSize:11,color:'var(--text-secondary)'}}>
                ...
                {a.sentimentLabel && (
                  <>
                    <span>·</span>
                    <Badge tone={a.sentimentLabel === 'positive' ? 'green' : a.sentimentLabel === 'negative' ? 'red' : 'gray'}>
                      {a.sentimentLabel}{typeof a.sentimentScore === 'number' ? ` ${a.sentimentScore >= 0 ? '+' : ''}${a.sentimentScore.toFixed(2)}` : ''}
                    </Badge>
                  </>
                )}
              </div>
