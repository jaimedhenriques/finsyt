...
type Tab = 'catalog' | 'connections' | 'custom' | 'routing'

const CATEGORY_ORDER = [
  'markets', 'fundamentals', 'macro', 'filings', 'news',
  'sentiment', 'crypto', 'fx', 'ai_nlp', 'geocoding',
  'calendars', 'comms', 'knowledge', 'crm', 'search',
]
...
interface RoutingDomainData {
  domain: string
  label: string
  description: string
  sources: PolicySourceClient[]
  availableSources: PolicySourceClient[]
}

function RoutingTab() {
  const [domains, setDomains] = useState<RoutingDomainData[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [err, setErr] = useState<string | null>(null)
  const [ok, setOk] = useState<string | null>(null)
  const [expanded, setExpanded] = useState<string | null>(null)
  // Per-domain dirty state: domain → PolicySourceClient[]
  const [edits, setEdits] = useState<Record<string, PolicySourceClient[]>>({})
  ...
  // Get the working source list for a domain (edit override or server state).
  function getSources(domain: string, serverSources: PolicySourceClient[]): PolicySourceClient[] {
    return edits[domain] !== undefined ? edits[domain] : serverSources
  }

  function moveSrc(domain: string, serverSources: PolicySourceClient[], fromIdx: number, toIdx: number) {
    const list = [...getSources(domain, serverSources)]
    const [item] = list.splice(fromIdx, 1)
    list.splice(toIdx, 0, item)
    setEdits(prev => ({ ...prev, [domain]: list }))
  }

  function addSrc(domain: string, serverSources: PolicySourceClient[], src: PolicySourceClient) {
    const list = getSources(domain, serverSources)
    if (list.find(s => s.id === src.id)) return
    setEdits(prev => ({ ...prev, [domain]: [...list, src] }))
  }

  function removeSrc(domain: string, serverSources: PolicySourceClient[], srcId: string) {
    const list = getSources(domain, serverSources).filter(s => s.id !== srcId)
    setEdits(prev => ({ ...prev, [domain]: list }))
  }

  function toggleDisabled(domain: string, serverSources: PolicySourceClient[], srcId: string) {
    const list = getSources(domain, serverSources).map(s =>
      s.id === srcId ? { ...s, disabled: !s.disabled } : s
    )
    setEdits(prev => ({ ...prev, [domain]: list }))
  }

  function setOpName(domain: string, serverSources: PolicySourceClient[], srcId: string, opName: string) {
    const list = getSources(domain, serverSources).map(s =>
      s.id === srcId ? { ...s, operationName: opName || undefined } : s
    )
    setEdits(prev => ({ ...prev, [domain]: list }))
  }

  async function saveDomain(domain: string, serverSources: PolicySourceClient[]) {
    const sources = getSources(domain, serverSources)
    setSaving(true); setErr(null); setOk(null)
    try {
      const res = await fetch(`${API}/routing/${domain}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      setOk(`${domain} routing policy saved.`)
      setEdits(prev => { const n = { ...prev }; delete n[domain]; return n })
      await load()
  ...
  async function resetDomain(domain: string) {
    setSaving(true); setErr(null); setOk(null)
    try {
      await fetch(`${API}/routing/${domain}`, { method: 'DELETE' })
      setEdits(prev => { const n = { ...prev }; delete n[domain]; return n })
      setOk(`${domain} reset to defaults.`)
      await load()
      setTimeout(() => setOk(null), 4000)
    } catch (e) {
      setErr((e as Error).message)
  ...
  if (loading) return <div className="py-12 text-center text-sm text-zinc-500">Loading routing policies…</div>

  return (
    <div>
      <div className="mb-5">
        <h2 className="text-base font-semibold">Source Routing</h2>
        <p className="mt-1 text-sm text-zinc-500">
          Configure which data sources Finsyt tries first for each domain. Sources are tried in order —
          if the first fails or returns empty data the next is tried automatically. Each served value
          is attributed to the source that provided it.
        </p>
      </div>
      ...
      {ok && (
        <div className="mb-4 rounded border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300">
          {ok}
        </div>
      )}
      ...
      <div className="space-y-3">
        {domains.map(d => {
          const currentSources = getSources(d.domain, d.sources)
          const isDirty = edits[d.domain] !== undefined
          const isOpen = expanded === d.domain
          const eligibleToAdd = d.availableSources.filter(s => !currentSources.find(c => c.id === s.id))
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-zinc-400">{currentSources.length} source{currentSources.length !== 1 ? 's' : ''}</span>
                </div>
              ...
              {isOpen && (
                <div className="border-t border-zinc-100 px-4 pb-4 pt-3 dark:border-zinc-800">
                  <p className="mb-3 text-xs text-zinc-500">{d.description}</p>

                  {/* Priority-ordered source list */}
                  {currentSources.length === 0 ? (
                    <p className="mb-3 text-xs italic text-zinc-400">No sources configured. Add sources from the picker below.</p>
                  ) : (
                    <div className="mb-3 space-y-1.5">
                      {currentSources.map((src, i) => (
                        ...
                        >
                          {/* Position badge */}
                          <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-zinc-100 text-[10px] font-bold text-zinc-500 dark:bg-zinc-700">
                            {src.disabled ? '—' : i + 1}
                          </span>
                          ...
                          <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider ${
                            ...
                            {src.type === 'connection' ? 'Connected' : 'Built-in'}
                          </span>

                          <span className="flex-1 font-medium">{src.label}</span>

                          {/* Operation name input (connections only) */}
                          {src.type === 'connection' && (
                            <input
                              value={src.operationName || ''}
                              onChange={e => setOpName(d.domain, d.sources, src.id, e.target.value)}
                              placeholder="operation name"
                              title="Operation to invoke on this connection for this domain"
                              className="w-36 rounded border border-zinc-200 bg-white px-2 py-0.5 text-xs font-mono dark:border-zinc-600 dark:bg-zinc-700"
                            />
                          )}
                          ...
                          <div className="flex gap-0.5">
                            <button
                              disabled={i === 0}
                              onClick={() => moveSrc(d.domain, d.sources, i, i - 1)}
                              className="rounded px-1.5 py-0.5 text-xs text-zinc-400 hover:bg-zinc-100 disabled:opacity-30 dark:hover:bg-zinc-700"
                              title="Move up"
                            >↑</button>
                            <button
                              disabled={i === currentSources.length - 1}
                              onClick={() => moveSrc(d.domain, d.sources, i, i + 1)}
                              className="rounded px-1.5 py-0.5 text-xs text-zinc-400 hover:bg-zinc-100 disabled:opacity-30 dark:hover:bg-zinc-700"
                              title="Move down"
                            >↓</button>
                          ...
                          {/* Toggle disabled */}
                          <button
                            onClick={() => toggleDisabled(d.domain, d.sources, src.id)}
                            className={`rounded px-1.5 py-0.5 text-xs ${
                              src.disabled
                                ? 'bg-zinc-100 text-zinc-500 hover:bg-zinc-200 dark:bg-zinc-700 dark:text-zinc-300'
                                : 'text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/30'
                            }`}
                            title={src.disabled ? 'Enable source' : 'Disable source (skip without removing)'}
                          >
                          ...
                          {/* Remove */}
                          <button
                            onClick={() => removeSrc(d.domain, d.sources, src.id)}
                            className="rounded px-1.5 py-0.5 text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30"
                  ...
                  {eligibleToAdd.length > 0 && (
                    <div className="mb-4">
                      <label className="mb-1 block text-xs font-medium text-zinc-500">Add source</label>
                      <div className="flex flex-wrap gap-1.5">
                        {eligibleToAdd.map(s => (
                          <button
                            key={s.id}
                            onClick={() => addSrc(d.domain, d.sources, s)}
                            className={`rounded border px-2 py-1 text-xs transition ${
                              s.type === 'connection'
                                ? 'border-violet-200 bg-violet-50 text-violet-700 hover:bg-violet-100 dark:border-violet-800 dark:bg-violet-950/30 dark:text-violet-300'
                                : 'border-zinc-200 bg-zinc-50 text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300'
                            }`}
                            title={`${s.type === 'connection' ? 'Connector' : 'Built-in'}: ${s.label}`}
                          >
                            + {s.label}
                          </button>
                        ))}
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={() => saveDomain(d.domain, d.sources)}
                    ...
                    {isDirty && (
                      <button
                        disabled={saving}
                        onClick={() => setEdits(prev => { const n = { ...prev }; delete n[d.domain]; return n })}
                        className="rounded border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-600 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
                      >
                        Discard
                      </button>
                    )}
                    <button
                      disabled={saving}
                      onClick={() => resetDomain(d.domain)}
                      className="ml-auto rounded border border-zinc-200 px-3 py-1.5 text-xs text-zinc-500 hover:bg-zinc-50 dark:border-zinc-700 dark:hover:bg-zinc-800"
      ...
      <div className="mt-6 rounded-lg border border-zinc-100 bg-zinc-50 px-4 py-3 text-xs text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900/50">
        <strong className="font-medium text-zinc-600 dark:text-zinc-300">How routing works:</strong>{' '}
        For each data domain, Finsyt tries sources in the order listed. The first source that returns data is used,
        and its name is cited in agent answers and the "Data sources used" footer. Disabled sources are skipped
        without losing their position. <em>Connection</em> sources (purple) use your federated licenses;
        <em> Built-in</em> sources (green) use Finsyt&apos;s built-in data provider waterfall.
      </div>
    </div>
  )
}
