...
export default function ConnectorHubPage() {
  ...
  const [validationFlash, setValidationFlash] = useState<
    | { ok: boolean; status: number; error?: string; latencyMs: number; name: string }
    | null
  >(null)
  useEffect(() => {
        ...
        >
          <span>
            {validationFlash.ok
              ...
              : `${validationFlash.name}: credentials rejected (HTTP ${validationFlash.status}). ${validationFlash.error || ''}`}
          </span>
          <button
...
// ─────────────────────────────────────────────────────────────────────────────
function ConnectModal({ entry, onClose, onCreated }: { entry: CatalogEntry; onClose: () => void; onCreated: (validation?: { ok: boolean; status: number; error?: string; latencyMs: number } | null) => void }) {
...
function ConnectionRow({ connection, expanded, onToggle, onChanged }: {
  ...
  async function test() {
    ...
    try {
      ...
      setTestResult(
        body.ok
          ? `OK${body.status ? ` · HTTP ${body.status}` : ''}${quotaSuffix}`
