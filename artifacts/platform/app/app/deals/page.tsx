...
export default function DealsPage() {
  ...
  return (
    <div className="page-content">
      ...
      <ContextualAskBar
        context="Deals & M&A"
        contextData={{ page: 'deals', total: filtered.length, totalValue }}
        chips={[
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:12, marginBottom:20 }}>
        {[
          { label:'Total Deals Shown', value: filtered.length,                                      color:'var(--accent)' },
          { label:'Total Value',       value: `$${(totalValue/1000).toFixed(0)}B`,                  color:'var(--text-primary)' },
          { label:'Pending',           value: filtered.filter(d=>d.status==='pending').length,      color:'var(--amber)' },
          { label:'Avg. Premium',      value: avgPremium > 0 ? `${avgPremium.toFixed(0)}%` : '—',  color:'var(--pos)' },
        ].map(card => (
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:16 }}>
        ...
        <select value={status} onChange={e => setStatus(e.target.value)}
          style={{ padding:'8px 12px', borderRadius:8, border:'1.5px solid var(--border)', fontSize:13, fontFamily:'inherit', color:'var(--text-primary)', background:'var(--bg-card)', cursor:'pointer', textTransform:'capitalize' }}>
          ...
          <tbody>
            {filtered.map(deal => (
              <tr key={deal.id} style={{ cursor:'pointer' }} onClick={() => setSelected(deal)}>
                ...
                <td className="right" style={{ fontSize:13, color: deal.premium && deal.premium > 0 ? 'var(--pos)' : 'var(--text-muted)', fontWeight:600 }}>
          ...
          <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:1001, width:560, maxWidth:'calc(100vw - 32px)', background:'var(--bg-card)', borderRadius:16, boxShadow:'0 16px 64px rgba(0,0,0,0.15)', overflow:'hidden', maxHeight:'80vh', overflowY:'auto' }}>
            ...
            <div style={{ padding:24 }}>
              ...
              <div style={{ display:'flex', gap:16, fontSize:12, color:'var(--text-secondary)' }}>
                <div><span style={{fontWeight:700,color:'var(--text-primary)'}}>Announced:</span> {selected.announced}</div>
                {selected.closed && <div><span style={{fontWeight:700,color:'var(--pos)'}}>Closed:</span> {selected.closed}</div>}
                {selected.advisor && <div><span style={{fontWeight:700,color:'var(--text-primary)'}}>Advisor:</span> {selected.advisor}</div>}
              </div>
              {/* Status badge */}
