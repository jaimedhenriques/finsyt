...
interface Alert {
  ...
  symbol: string
  name: string
  type: 'price_above' | 'price_below' | 'pct_change' | 'volume_spike' | 'news' | 'news_sentiment'
  threshold: number
  currentVal: number
  triggered: boolean
  active: boolean
...
const ALERT_TYPES: { value: AlertType; label: string; desc: string }[] = [
  ...
  { value: 'news_sentiment', label: 'News Sentiment', desc: 'Alert when news sentiment or volume deviates from baseline (σ)' },
]
...
export default function AlertsPage() {
  ...
  const checkPrices = async () => {
    ...
    try {
      ...
      // News-sentiment alerts evaluate the trailing-baseline deviation verdict
      // from /api/news/sentiment (llm=0 keeps this poll cheap). currentVal
      // holds the latest mean sentiment; the alert fires when the deviation
      // core flags a sentiment swing or volume spike beyond `threshold` σ.
      const sentSymbols = [...new Set(
        alerts.filter(a => a.type === 'news_sentiment' && a.active).map(a => a.symbol),
      )]
      const sentMap: Record<string, { avgScore: number; signal: boolean; maxZ: number }> = {}
      if (sentSymbols.length) {
        const sentResults = await Promise.allSettled(
          sentSymbols.map(sym =>
            fetch(`/api/news/sentiment?symbol=${encodeURIComponent(sym)}&days=30&llm=0`)
              .then(r => r.json())
              .then(d => ({ sym, d })),
          ),
        )
        sentResults.forEach(r => {
          if (r.status !== 'fulfilled' || !r.value.d || r.value.d.error) return
          const dv = r.value.d.deviation
          if (!dv) return
          const maxZ = Math.max(Math.abs(dv.sentiment?.z ?? 0), dv.volume?.z ?? 0)
          sentMap[r.value.sym] = {
            avgScore: r.value.d.current?.avgScore ?? 0,
            signal: !!dv.hasSignal,
            maxZ,
          }
        })
      ...
      setAlerts(prev => {
        const next = prev.map(a => {
          ...
          if (a.type === 'news_sentiment') {
            const s = sentMap[a.symbol]
            if (!s) return a
            const threshold = a.threshold > 0 ? a.threshold : 2
            const nowTriggered = s.signal && s.maxZ >= threshold
            const flippedTrue = nowTriggered && !a.triggered
            if (flippedTrue) {
              queueMicrotask(() => fireNotification({ ...a, currentVal: s.avgScore, triggered: true }))
            }
            if (s.avgScore !== a.currentVal || flippedTrue) {
              patchAlertServer(a.id, {
                currentVal: s.avgScore,
                triggered: nowTriggered || a.triggered,
                lastCheckedAt: checkedAt,
              })
            }
            return { ...a, currentVal: s.avgScore, triggered: nowTriggered || a.triggered, lastChecked: checkedAt }
          }
              ...
              <button key={a.id} onClick={() => setSelectedId(a.id)} className="card" style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>
                    {typeLabel(a.type)} {a.type === 'news_sentiment' ? `· ≥${a.threshold > 0 ? a.threshold : 2}σ` : a.type !== 'volume_spike' && a.type !== 'news' ? `· ${a.type === 'pct_change' ? a.threshold + '%' : '$' + fmt(a.threshold)}` : ''}
                  </div>
                </div>
              </button>
            )
          ...
          {!selected ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
                ...
                <Field label="Threshold" value={selected.type === 'news_sentiment' ? `≥${selected.threshold > 0 ? selected.threshold : 2}σ` : selected.type === 'pct_change' ? `${selected.threshold}%` : selected.type === 'volume_spike' ? '2× avg' : selected.type === 'news' ? '—' : `$${fmt(selected.threshold)}`} />
                <Field label="Current" value={selected.type === 'news_sentiment' ? `${selected.currentVal >= 0 ? '+' : ''}${selected.currentVal.toFixed(2)} sentiment` : `$${fmt(quotes[selected.symbol] || selected.currentVal)}`} />
        ...
        <div onClick={() => setShowNew(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <Card onClick={(e: React.MouseEvent) => e.stopPropagation()} padding={24} style={{ width: '100%', maxWidth: 460, border: '1.5px solid var(--accent)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              ...
              <div>
                ...
                <Select value={form.type} onChange={e => {
                  ...
                  setForm(f => ({ ...f, type, threshold: type === 'news_sentiment' && (!f.threshold || Number(f.threshold) > 6) ? '2' : f.threshold }))
                }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <FieldLabel>{form.type === 'news_sentiment' ? 'Deviation threshold (σ)' : 'Threshold'}</FieldLabel>
              <Input type="number" value={form.threshold} onChange={e => setForm(f => ({ ...f, threshold: e.target.value }))} placeholder={form.type === 'news_sentiment' ? '2' : '150'} />
              {form.type === 'news_sentiment' && (
                <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4 }}>
                  Fires when daily news sentiment or volume deviates by at least this many standard deviations from its trailing baseline.
                </div>
              )}
            </div>
            <div>
