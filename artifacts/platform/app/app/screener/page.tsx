...
import { Card, Badge, DataTable, ContextualAskBar, InlineAgentMenu, Drawer, type DataColumn } from '@/components/ui'
...
export default function ScreenerPage() {
  ...
  // Focus ticker for the alt-data cards — defaults to the top result and
  // resets when it falls out of the filtered set. One ticker → one Apify run.
  const { focusSymbol, setFocusSymbol, reconcileFocus } = usePersistentFocusSymbol('finsyt.focus.screener')
  useEffect(() => {
    reconcileFocus(filtered.map(s => s.symbol))
  }, [filtered, reconcileFocus])
  const focusName = focusSymbol ? (filtered.find(s => s.symbol === focusSymbol)?.name || focusSymbol) : ''
      ...
      <div style={{display:'grid',gridTemplateColumns:'280px 1fr',gap:16,alignItems:'start'}}>
        ...
        <Card padding={0} style={{overflow:'hidden'}}>
          ...
          <DataTable
            ...
            onRowClick={s => setFocusSymbol(s.symbol)}
            isRowActive={s => s.symbol === focusSymbol}
        ...
        {focusSymbol && (
          <div style={{marginTop:18}}>
            <FocusPicker label="Alt-data for" symbols={filtered.map(s=>s.symbol)} value={focusSymbol} onChange={setFocusSymbol} />
