...
import type { VerificationReport, VerificationIssue, IssueSeverity } from '@/lib/deliverable-verification'
...
export default function MatrixPage() {
  ...
  // Pre-export verification (Task #519). `review` holds the report + the
  // pending export format so the modal can run the export once the analyst
  // confirms. `reviewLoading` gates the spinner while /verify is in flight.
  const [review, setReview] = useState<{
    report: VerificationReport
    pendingFormat: 'csv' | 'pptx'
  ...
  const runCell = useCallback(async (rowId: string, colId: string) => {
    ...
    const provider = 'openai-via-finsyt'
  ...
  // Pre-export verification (Task #519): run the deliverable verification
  // engine server-side, then surface a review pane with click-through before
  // the analyst commits to the export. PPTX is the banker-facing artifact, so
  // it gets the review gate; CSV stays a direct download.
  async function reviewBeforeExport(format: 'csv' | 'pptx', opts?: { scopeToSelected?: boolean }) {
    const id = await ensureMatrix()
    if (!id) { setStatusMsg('Sign in to export.'); return }
    const scopeToSelected = !!opts?.scopeToSelected && selected.size > 0
    setReviewLoading(true)
    try {
      const params = new URLSearchParams()
      if (scopeToSelected) for (const rid of selected) params.append('rowIds', rid)
      const qs = params.toString()
      const r = await fetch(`${BASE}/api/matrices/${id}/verify${qs ? `?${qs}` : ''}`)
      if (!r.ok) {
        // Verification is a safety net, not a blocker — fall back to a direct
        // export so a transient verify failure never traps the analyst.
        setStatusMsg(`Verification unavailable (${r.status}); exporting without review.`)
        await exportFile(format, { scopeToSelected })
        return
      }
      const { report } = (await r.json()) as { report: VerificationReport }
      if (!report || !report.summary) {
        setStatusMsg('Verification returned no report; exporting without review.')
        await exportFile(format, { scopeToSelected })
        return
      }
      setReview({ report, pendingFormat: format, scopeToSelected })
    } catch {
      setStatusMsg('Verification failed; exporting without review.')
      await exportFile(format, { scopeToSelected })
    } finally {
    ...
    <div style={{ color: 'var(--text-primary)' }}>
          ...
          actions={
            <>
              ...
              <DelegateButton
                ...
                context={{ surface: 'matrix', matrixId: matrixId ?? undefined, label: name, defaultDeliverable: 'matrix' }}
              />
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 16px', marginBottom: 14, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button
                ...
                onClick={() => reviewBeforeExport('pptx', { scopeToSelected: true })}
                title={selected.size > 0
                  ? `Verify, then build a Finsyt-branded PPTX from the ${selected.size} row${selected.size === 1 ? '' : 's'} you've selected`
                  : 'Verify, then build a Finsyt-branded PPTX from this matrix view (check rows to export a subset)'}
              >
                {reviewLoading ? '… Verifying' : (selected.size > 0 ? `⤓ Review & PPTX (${selected.size} selected)` : '⤓ Review & PPTX')}
              </button>
            </div>
          </div>
      ...
      {review && (
        <ExportReviewModal
          ...
          onJumpToCell={(rowId, colId) => {
            setReview(null)
            setActiveCell({ row: rowId, col: colId })
          }}
          onExport={async () => {
            const fmt = review.pendingFormat
            const scope = review.scopeToSelected
            setReview(null)
            await exportFile(fmt, { scopeToSelected: scope })
          }}
        />
      )}
...
// ─── Pre-export review modal (Task #519) ─────────────────────────────────────
// Renders the deliverable verification report before the analyst commits to a
// PPTX export. Each cell-scoped issue is click-through: jumping to a flagged
// cell closes the modal and opens that cell's drawer in the grid.

const SEV_META: Record<IssueSeverity, { color: string; bg: string; label: string; icon: string }> = {
  error:   { color: '#D9434E', bg: 'rgba(217,67,78,0.12)',  label: 'Error',   icon: '⛔' },
  warning: { color: '#B7791F', bg: 'rgba(183,121,31,0.12)', label: 'Warning', icon: '⚠' },
  info:    { color: '#4F7CFF', bg: 'rgba(79,124,255,0.12)', label: 'Info',    icon: 'ℹ' },
}

function ExportReviewModal({
  report, onClose, onExport, onJumpToCell,
}: {
  report: VerificationReport
  onClose: () => void
  onExport: () => void | Promise<void>
  onJumpToCell: (rowId: string, colId: string) => void
}) {
  useEffect(() => {
  ...
  return (
    <div
      onClick={onClose}
      style={{
        position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(8,12,24,0.55)',
      ...
      >
        ...
        <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={onClose} style={{ ...iconBtn, fontSize: 18 }} aria-label="Close">×</button>
          </div>
          {/* Summary chips */}
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}>
            <span style={chipStyle(clean ? '#0EA371' : (blocking ? '#D9434E' : '#B7791F'))}>
              {clean ? '✓ No issues found' : `${summary.total} issue${summary.total === 1 ? '' : 's'}`}
            </span>
            {summary.error > 0 && <span style={chipStyle('#D9434E')}>{summary.error} error{summary.error === 1 ? '' : 's'}</span>}
            {summary.warning > 0 && <span style={chipStyle('#B7791F')}>{summary.warning} warning{summary.warning === 1 ? '' : 's'}</span>}
            {summary.info > 0 && <span style={chipStyle('#4F7CFF')}>{summary.info} info</span>}
            {summary.autoFixable > 0 && <span style={chipStyle('#6B7280')}>{summary.autoFixable} auto-fixable</span>}
            <span style={chipStyle(report.houseStyleApplied ? '#4F7CFF' : '#6B7280')}>
              {report.houseStyleApplied ? '◆ House style on' : '◇ House style off'}
            </span>
          </div>
        </div>

        {/* Issue list */}
        <div style={{ overflowY: 'auto', padding: clean ? 0 : '12px 16px' }}>
          {clean ? (
            <div style={{ padding: '36px 22px', textAlign: 'center', color: 'var(--text-secondary)', fontSize: 13 }}>
              This deliverable passed every verification check. You&apos;re good to export.
            </div>
          ) : (
            issues.map((issue) => (
              <IssueRow key={issue.id} issue={issue} onJumpToCell={onJumpToCell} />
            ))
          )}
        </div>
        ...
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
            {blocking
              ? 'Errors won\u2019t block export, but review them first — they usually mean a cell failed or numbers disagree.'
              : 'Export proceeds with your current grid. Auto-fixable items are corrected on export.'}
          </div>
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            <button onClick={onClose} style={ghostBtn}>Cancel</button>
            <button onClick={() => { void onExport() }} style={primaryBtn}>
              {blocking ? 'Export anyway' : 'Export PPTX'}
            </button>
          </div>
        </div>
      </div>
...
function IssueRow({
  ...
  const jumpable = issue.location.kind === 'cell' && !!issue.location.rowId && !!issue.location.columnId
  return (
    <div style={{
      display: 'flex', gap: 12, padding: '12px 14px', marginBottom: 8, borderRadius: 10,
      background: 'var(--bg-elevated)', border: '1px solid var(--border)',
    }}>
      <div style={{
        flexShrink: 0, width: 26, height: 26, borderRadius: 7, display: 'flex', alignItems: 'center',
        justifyContent: 'center', background: meta.bg, color: meta.color, fontSize: 13,
      }}>{meta.icon}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{issue.title}</span>
          <span style={{ fontSize: 10, fontWeight: 700, color: meta.color, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{meta.label}</span>
          <span style={{ fontSize: 10, color: 'var(--text-muted)', background: 'var(--bg-base)', border: '1px solid var(--border)', borderRadius: 4, padding: '1px 6px' }}>
            {issue.category.replace(/-/g, ' ')}
          </span>
          {issue.autoFixable && (
            <span style={{ fontSize: 10, fontWeight: 700, color: '#0EA371' }}>auto-fix on export</span>
          )}
        </div>
        {issue.location.label && (
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{issue.location.label}</div>
        )}
        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.45 }}>{issue.detail}</div>
        {issue.autoFix?.replacement && (
          <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4 }}>
            Suggested: <code style={{ background: 'var(--bg-base)', padding: '1px 5px', borderRadius: 3 }}>{issue.autoFix.replacement}</code>
          </div>
        )}
      </div>
      {jumpable && (
        <button
          onClick={() => onJumpToCell(issue.location.rowId!, issue.location.columnId!)}
          style={{ ...miniBtn, flexShrink: 0, alignSelf: 'flex-start' }}
          title="Jump to this cell in the grid"
        >
          Go to cell →
        </button>
      )}
...
function chipStyle(color: string): React.CSSProperties {
  return {
    fontSize: 11, fontWeight: 700, color, padding: '3px 9px', borderRadius: 999,
