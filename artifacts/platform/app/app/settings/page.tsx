...
type Section = 'account' | 'team' | 'billing' | 'data' | 'notifications' | 'appearance' | 'api'
...
// ── Domain Sources Picker ─────────────────────────────────────────────────────
// Shows each data domain (quotes, fundamentals, estimates, …) with a dropdown
// of available connections. When the user selects a custom connection as the
// authoritative source, the page renders strictly from that source with no
// Finsyt backfill. A "Map Fields" link leads to the field-mapping review UI.
...
const DOMAIN_LABELS: Record<string, string> = {
  quotes: 'Quotes & Prices', fundamentals: 'Fundamentals', estimates: 'Estimates & Consensus',
  news: 'News', filings: 'Filings', transcripts: 'Earnings Transcripts',
  macro: 'Macro / Economic', ownership: 'Institutional Ownership', deals: 'M&A Deals',
}
...
interface RoutingRow { domain: string; sources: PolicySource[]; isDefault: boolean }
interface ConnectionOpt { id: string; displayName: string; status: string; category: string }

function DomainSourcesPicker() {
  const [policies, setPolicies] = useState<Record<string, RoutingRow>>({})
  const [connections, setConnections] = useState<ConnectionOpt[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState<string | null>(null)
  const [msg, setMsg] = useState<{ domain: string; ok: boolean; text: string } | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        const [connRes, ...policyRes] = await Promise.all([
          fetch(`${API}/connections`),
          ...ROUTING_DOMAINS.map(d => fetch(`${API}/routing/${d}`)),
        ])
        if (!alive) return
        const connData = await connRes.json()
        if (alive) setConnections(Array.isArray(connData) ? connData : connData.connections ?? [])
        const map: Record<string, RoutingRow> = {}
        for (let i = 0; i < ROUTING_DOMAINS.length; i++) {
          const d = ROUTING_DOMAINS[i]
          const r = policyRes[i]
          if (r.ok) { map[d] = await r.json() }
          else { map[d] = { domain: d, sources: [], isDefault: true } }
        }
        if (alive) setPolicies(map)
      } catch {
  ...
  function currentSourceId(domain: string): string {
    const pol = policies[domain]
    if (!pol) return 'finsyt'
    const first = pol.sources.find(s => !s.disabled)
    if (!first || first.type === 'builtin') return 'finsyt'
    return first.id
  }

  async function setSource(domain: string, connectionId: string) {
    setSaving(domain); setMsg(null)
    try {
      let sources: PolicySource[]
      if (connectionId === 'finsyt') {
        sources = [{ type: 'builtin', id: 'finsyt', label: 'Finsyt Default' }]
      } else {
        const conn = connections.find(c => c.id === connectionId)
        sources = [
          { type: 'connection', id: connectionId, label: conn?.displayName ?? connectionId, operationName: 'query' },
          { type: 'builtin', id: 'finsyt', label: 'Finsyt Default', disabled: true },
        ]
      }
      const r = await fetch(`${API}/routing/${domain}`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
      ...
      const d = await r.json()
      if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`)
      setPolicies(prev => ({ ...prev, [domain]: { domain, sources, isDefault: false } }))
      setMsg({ domain, ok: true, text: connectionId === 'finsyt' ? 'Reset to Finsyt default' : 'Source updated — configure field mapping to go live' })
    } catch (e: unknown) {
      setMsg({ domain, ok: false, text: (e as Error).message })
    } finally {
      setSaving(null)
      setTimeout(() => setMsg(prev => prev?.domain === domain ? null : prev), 5000)
    }
  }

  const warehouseConnections = connections.filter(c =>
    c.category === 'warehouse' || c.status === 'active'
  )

  return (
    <div className="card" style={{ overflow:'hidden' }}>
      <div style={{ padding:'16px 20px', borderBottom:'1px solid #E2E8F2', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          <div style={{ fontWeight:800, fontSize:16, color:'#0A1628', display:'flex', alignItems:'center', gap:8 }}>
            Bring Your Own Source
            <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20, background:'rgba(27,79,255,0.08)', color:'#1B4FFF', letterSpacing:'0.04em', textTransform:'uppercase' }}>
              Enterprise
            </span>
          </div>
          <div style={{ fontSize:12, color:'#7D8FA9', marginTop:4, maxWidth:560, lineHeight:1.5 }}>
            Select an authoritative data source for each domain. When set, pages render strictly from that source — no Finsyt backfill.
            Connect a Snowflake, Databricks, SQL, or FTP source in the{' '}
            <a href="/platform/app/connectors" style={{ color:'#1B4FFF', textDecoration:'none', fontWeight:600 }}>Connector Hub</a>.
          </div>
        </div>
        {connections.length === 0 && !loading && (
          <a href="/platform/app/connectors?category=warehouse"
            style={{ padding:'8px 14px', borderRadius:8, border:'1.5px solid #1B4FFF', background:'#fff', color:'#1B4FFF', fontSize:12, fontWeight:700, textDecoration:'none', flexShrink:0 }}>
            + Connect Source
          </a>
        )}
      </div>
      {loading ? (
        <div style={{ padding:'18px 20px', fontSize:13, color:'#7D8FA9' }}>Loading domain policies…</div>
      ) : (
        <div>
          {ROUTING_DOMAINS.map((domain, i) => {
            const current = currentSourceId(domain)
            const isBusy = saving === domain
            const domainMsg = msg?.domain === domain ? msg : null
            return (
              <div key={domain} style={{ padding:'14px 20px', borderBottom: i < ROUTING_DOMAINS.length - 1 ? '1px solid #F0F4FA' : 'none' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  <span style={{ fontSize:13, color:'#B0BCD0', flexShrink:0 }}>{DOMAIN_ICONS[domain]}</span>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:700, fontSize:13, color:'#0A1628' }}>{DOMAIN_LABELS[domain]}</div>
                  </div>
                  <select
                    value={current}
                    ...
                    disabled={isBusy}
                    style={{
                      padding:'6px 10px', borderRadius:8, border:'1.5px solid #E2E8F2',
                      fontSize:12, fontFamily:'inherit', background:'#fff', color:'#0A1628',
                      cursor:'pointer', minWidth:200, maxWidth:260,
                      borderColor: current !== 'finsyt' ? '#1B4FFF' : '#E2E8F2',
                    }}>
                    <option value="finsyt">Finsyt Default (waterfall)</option>
                    {warehouseConnections.length > 0 && (
                      <optgroup label="Your Connections">
                        {warehouseConnections.map(c => (
                  ...
                  {current !== 'finsyt' && (
                    <a
                      href={`/platform/app/connectors/mappings?connectionId=${current}&domain=${domain}`}
                      style={{ padding:'6px 12px', borderRadius:7, border:'1px solid #E2E8F2', background:'#F8FAFD', color:'#4A5568', fontSize:12, fontWeight:600, textDecoration:'none', flexShrink:0 }}>
                      Map Fields →
                    </a>
                  )}
                  {current === 'finsyt' && (
                    <span style={{ fontSize:11, padding:'2px 10px', borderRadius:20, background:'rgba(176,188,208,0.15)', color:'#B0BCD0', fontWeight:600, whiteSpace:'nowrap' }}>
                      default
                    </span>
                  )}
                  {isBusy && <span style={{ fontSize:11, color:'#7D8FA9' }}>Saving…</span>}
                </div>
                {domainMsg && (
                  <div style={{ marginTop:6, marginLeft:25, fontSize:12, fontWeight:600,
                    color: domainMsg.ok ? '#059669' : '#DC2626' }}>
                    {domainMsg.ok ? '✓ ' : '⚠ '}{domainMsg.text}
                  </div>
                )}
              </div>
            )
      ...
      <div style={{ padding:'12px 20px', background:'#F8FAFD', borderTop:'1px solid #F0F4FA', fontSize:12, color:'#7D8FA9', lineHeight:1.6 }}>
        Once you select a custom source, use <strong>Map Fields</strong> to auto-map your column names to Finsyt canonical datapoints, then confirm.
        The platform will render only what your source provides — required fields marked with ✶ unlock the full page experience.
      </div>
    </div>
  )
}
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const allowed: readonly Section[] = ['account','team','billing','data','notifications','appearance','api']
  ...
  const {
    theme, setTheme,
    dataSourcesFooterEnabled, setDataSourcesFooterEnabled,
    dataSourcesFooterCollapsed, setDataSourcesFooterCollapsed,
  } = useWorkspace()
      ...
      <div style={{ display:'flex', gap:24, alignItems:'flex-start' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          {section === 'data' && (
            ...
            {/* BYOS domain routing picker */}
            <DomainSourcesPicker />

            {/* Built-in provider status */}
            <div className="card" style={{ overflow:'hidden' }}>
              <div style={{ padding:'16px 20px', borderBottom:'1px solid #E2E8F2' }}>
                <div style={{ fontWeight:800, fontSize:16, color:'#0A1628' }}>Built-in Data Providers</div>
              ...
              <div style={{ padding:'12px 20px', background:'#F8FAFD', borderTop:'1px solid #F0F4FA' }}>
                <div style={{ fontSize:12, color:'#7D8FA9' }}>
                  API keys are configured as Replit environment variables and never exposed client-side.
                </div>
              </div>
            </div>
            <ConnectSourcesPanel />
          ...
          {section === 'notifications' && (
            <div className="card" style={{ padding:24 }}>
              ...
              {/* Live Highlights blueprint link */}
              <div style={{
                marginTop:18, padding:'14px 16px', borderRadius:10,
                border:'1px solid #E2E8F2', background:'#F7F9FC',
                display:'flex', alignItems:'center', justifyContent:'space-between',
              }}>
                <div style={{ paddingRight:16 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#0A1628' }}>◉ Live Highlights</div>
                  <div style={{ fontSize:12, color:'#7D8FA9', marginTop:2, lineHeight:1.5 }}>
                    Auto-pin management commentary, KPI changes and Q&amp;A standouts from live earnings calls
                    on your watchlist. Configure the Blueprint, opt symbols out, or add ad-hoc tickers.
                  </div>
                </div>
                <a href={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/app/settings/live-highlights`}
                  style={{ padding:'7px 12px', borderRadius:8, border:'1.5px solid var(--accent)',
                    color:'var(--accent)', background:'#fff', fontSize:12, fontWeight:700,
          ...
          {section === 'appearance' && (
            <div className="card" style={{ padding:24 }}>
              ...
              {/* Agent answer transparency — controls whether every Finsyt
                  agent answer appends a "Data sources used" footer listing
                  every provider/connector that fed the response. */}
              <div style={{ marginTop:24, paddingTop:20, borderTop:'1px solid var(--border)' }}>
                <div style={{ fontSize:12, fontWeight:700, color:'var(--text-secondary)', marginBottom:14, letterSpacing:'0.04em', textTransform:'uppercase' }}>Agent transparency</div>
                {[
                  {
                    label: "Show 'Data sources used' footer",
                    desc: 'Every Finsyt agent answer appends a footer listing each provider with role, response time, and a deep link to the Connector Hub.',
                    value: dataSourcesFooterEnabled,
                    onToggle: () => setDataSourcesFooterEnabled(!dataSourcesFooterEnabled),
                    disabled: false,
                  },
                  {
                    label: 'Collapse footer by default',
                    desc: "Footer renders collapsed; click 'Show ▾' on any answer to expand. Per-answer state still wins after manual toggle.",
                    value: dataSourcesFooterCollapsed,
                    onToggle: () => setDataSourcesFooterCollapsed(!dataSourcesFooterCollapsed),
                    disabled: !dataSourcesFooterEnabled,
                  },
                ].map((t, i, arr) => (
                  <div key={t.label} data-testid={`dsf-toggle-${i}`} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 0', borderBottom: i < arr.length - 1 ? '1px solid #F0F4FA' : 'none', opacity: t.disabled ? 0.5 : 1 }}>
                    <div>
                      <div style={{ fontWeight:600, fontSize:13, color:'#0A1628' }}>{t.label}</div>
                      <div style={{ fontSize:12, color:'#7D8FA9', marginTop:2, maxWidth:520 }}>{t.desc}</div>
                    </div>
                    <button onClick={t.onToggle} disabled={t.disabled} aria-pressed={t.value}
                      style={{ width:44, height:24, borderRadius:12, border:'none', cursor: t.disabled ? 'not-allowed' : 'pointer', transition:'all 0.2s', position:'relative', flexShrink:0,
                        background: t.value ? 'var(--accent)' : 'var(--border)' }}>
                      <div style={{ width:18, height:18, borderRadius:'50%', background:'#fff', position:'absolute', top:3, transition:'left 0.2s', left: t.value ? 23 : 3, boxShadow:'0 1px 4px rgba(0,0,0,0.15)' }} />
                    </button>
                  </div>
                ))}
              </div>
...
function TeamSection(p: TeamSectionProps) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      {/* Enterprise SSO & SCIM info card (visible to enterprise admins/owners only) */}
      {callerIsAdmin && p.isEnterpriseTier && (
        <div className="card" style={{ padding:20, border:'1px solid #E2E8F2' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
            <span style={{ fontSize:14, fontWeight:800, color:'#0A1628' }}>Enterprise SSO & SCIM</span>
            <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20, background:'rgba(27,79,255,0.08)', color:'#1B4FFF', letterSpacing:'0.04em', textTransform:'uppercase' }}>Enterprise</span>
          </div>
          <p style={{ fontSize:12, color:'#7D8FA9', lineHeight:1.6, marginBottom:12 }}>
            Enable SAML SSO to let your directory provider authenticate users automatically.
            SCIM provisioning syncs directory group changes into Finsyt memberships in real time &mdash; no manual invites needed.
          </p>
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:'#4A5568' }}>
              <span style={{ width:8, height:8, borderRadius:'50%', flexShrink:0, background: process.env.NEXT_PUBLIC_ENTERPRISE_SSO_CONFIGURED === '1' ? '#059669' : '#D1D5DB', display:'inline-block' }} />
              <span>SAML connection: {process.env.NEXT_PUBLIC_ENTERPRISE_SSO_CONFIGURED === '1' ? 'configured' : 'not configured — set up in Clerk Dashboard'}</span>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:'#4A5568' }}>
              <span style={{ width:8, height:8, borderRadius:'50%', flexShrink:0, background: process.env.NEXT_PUBLIC_SCIM_CONFIGURED === '1' ? '#059669' : '#D1D5DB', display:'inline-block' }} />
              <span>SCIM webhook: {process.env.NEXT_PUBLIC_SCIM_CONFIGURED === '1' ? 'active' : 'not configured — register webhook endpoint in Clerk Dashboard'}</span>
            </div>
          </div>
          <div style={{ marginTop:14, display:'flex', gap:8, flexWrap:'wrap' }}>
            <a
              ...
              style={{ padding:'7px 14px', borderRadius:8, border:'1px solid #E2E8F2', background:'#fff', color:'#1B4FFF', fontSize:12, fontWeight:600, textDecoration:'none', cursor:'pointer' }}
            >
              SAML setup guide →
            </a>
            <a
              href="https://clerk.com/docs/users/sync-data"
              target="_blank"
              rel="noopener noreferrer"
              style={{ padding:'7px 14px', borderRadius:8, border:'1px solid #E2E8F2', background:'#fff', color:'#1B4FFF', fontSize:12, fontWeight:600, textDecoration:'none', cursor:'pointer' }}
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              style={{ padding:'7px 14px', borderRadius:8, border:'1px solid #E2E8F2', background:'#fff', color:'#4A5568', fontSize:12, fontWeight:600, textDecoration:'none', cursor:'pointer' }}
            >
              Clerk Dashboard ↗
            </a>
          </div>
          <p style={{ fontSize:11, color:'#B0BCD0', marginTop:12, lineHeight:1.5 }}>
            Enterprise-provisioned members appear in the table above. Role changes in your directory propagate automatically via the SCIM webhook. To remove SAML access, deactivate or suspend the user in your IdP.
          </p>
        </div>
      )}
      {callerIsAdmin && !p.isEnterpriseTier && (
        <div className="card" style={{ padding:20, border:'1px solid #E2E8F2', background:'#F8FAFD' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
            <span style={{ fontSize:14, fontWeight:800, color:'#0A1628' }}>Enterprise SSO & SCIM</span>
            <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20, background:'rgba(27,79,255,0.08)', color:'#1B4FFF', letterSpacing:'0.04em', textTransform:'uppercase' }}>Enterprise</span>
          </div>
          <p style={{ fontSize:12, color:'#7D8FA9', lineHeight:1.6, marginBottom:12 }}>
            SAML SSO and SCIM directory provisioning are available on the Enterprise plan.
            Upgrade to enable automatic user provisioning from Okta, Azure AD, or any SAML 2.0 provider.
          </p>
          <a
            href="mailto:sales@finsyt.com?subject=Enterprise%20SSO"
            style={{ padding:'7px 14px', borderRadius:8, border:'1px solid #1B4FFF', background:'#1B4FFF', color:'#fff', fontSize:12, fontWeight:600, textDecoration:'none', cursor:'pointer', display:'inline-block' }}
          >
...
function BillingSection() {
  ...
  async function toggleCancel() {
    ...
    try {
      const res = await fetch('/platform/api/billing/cancel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ resume: data.cancelAtPeriodEnd }),
      })
  ...
  if (loading) {
    return <div className="card" style={{ padding:'20px' }}><div style={{ fontSize:13, color:'#7D8FA9' }}>Loading billing…</div></div>
  }

  const plan = data?.plan ?? 'free'
  const isPaid = plan === 'pro' || plan === 'enterprise'
  const usage = data?.usage.aiQueries

  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      <div className="card" style={{ overflow:'hidden' }}>
        <div style={{ padding:'16px 20px', borderBottom:'1px solid #E2E8F2', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
          <div>
            <div style={{ fontWeight:800, fontSize:16, color:'#0A1628' }}>Billing & Plan</div>
            <div style={{ fontSize:12, color:'#7D8FA9', marginTop:4 }}>Manage your workspace subscription and usage.</div>
          </div>
          <span style={{ fontSize:12, fontWeight:800, padding:'4px 14px', borderRadius:20,
            background: isPaid ? 'rgba(27,79,255,0.08)' : 'rgba(125,143,169,0.1)',
            color: isPaid ? 'var(--accent)' : '#7D8FA9' }}>
            {PLAN_LABEL[plan]}{data?.demoMode ? ' · DEMO' : ''}
          </span>
        </div>

        <div style={{ padding:'18px 20px', display:'flex', flexDirection:'column', gap:14 }}>
          {!data?.billingConfigured && !data?.demoMode && (
            <div style={{ fontSize:13, color:'var(--amber)', background:'rgba(217,119,6,0.08)', borderRadius:8, padding:'10px 14px' }}>
              Billing is not configured on this environment. Subscription management is unavailable.
            </div>
          )}
          ...
          {usage && usage.limit !== null && (
            <div>
              <div style={{ fontSize:13, color:'#4A5568', marginBottom:6 }}>
                AI research queries this month: <strong style={{ color:'#0A1628' }}>{usage.used}</strong> / {usage.limit}
              </div>
              <div style={{ height:6, borderRadius:99, background:'#E2E8F2', overflow:'hidden' }}>
                <div style={{ height:'100%', width:`${Math.min(100, (usage.used / usage.limit) * 100)}%`,
                  background: usage.remaining === 0 ? 'var(--neg)' : 'var(--accent)' }} />
              </div>
              {usage.remaining === 0 && (
                <div style={{ fontSize:12, color:'var(--amber)', marginTop:6 }}>You’ve hit the Free monthly limit. Upgrade for unlimited research.</div>
              )}
            </div>
          )}
          {usage && usage.limit === null && (
            <div style={{ fontSize:13, color:'#4A5568' }}>Unlimited AI research queries on your current plan.</div>
          )}

          {isPaid && data?.currentPeriodEnd && (
            <div style={{ fontSize:13, color:'#4A5568' }}>
              {data.cancelAtPeriodEnd ? 'Access ends' : 'Renews'} on{' '}
              <strong style={{ color:'#0A1628' }}>{new Date(data.currentPeriodEnd).toLocaleDateString()}</strong>
            </div>
          )}

          <div style={{ display:'flex', gap:10, flexWrap:'wrap', marginTop:4 }}>
            {!isPaid && (
              <a href="/platform/app/upgrade" style={{ textDecoration:'none' }}>
                <button style={{ background:'var(--accent)', color:'#fff', border:'none', borderRadius:8, padding:'10px 18px', fontSize:13, fontWeight:700, cursor:'pointer' }}>Upgrade plan</button>
              </a>
            )}
            {isPaid && !data?.demoMode && data?.billingConfigured && data?.status !== 'none' && (
              <button onClick={toggleCancel} disabled={busy}
                style={{ background:'#fff', color:'#0A1628', border:'1px solid #E2E8F2', borderRadius:8, padding:'10px 18px', fontSize:13, fontWeight:600, cursor: busy ? 'default' : 'pointer', opacity: busy ? 0.6 : 1 }}>
                {data?.cancelAtPeriodEnd ? 'Resume subscription' : 'Cancel subscription'}
              </button>
            )}
            <a href="/platform/app/upgrade" style={{ textDecoration:'none' }}>
              <button style={{ background:'#fff', color:'#4A5568', border:'1px solid #E2E8F2', borderRadius:8, padding:'10px 18px', fontSize:13, fontWeight:600, cursor:'pointer' }}>View all plans</button>
            </a>
          </div>
        </div>
      </div>
