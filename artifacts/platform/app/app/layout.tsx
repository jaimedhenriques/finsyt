...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Gated to non-production so we never seed demo rows into a live DB even
  // if someone leaves PLATFORM_OPEN_MODE=1 set in a prod environment.
  if (OPEN_MODE && process.env.NODE_ENV !== 'production') {
    try {
