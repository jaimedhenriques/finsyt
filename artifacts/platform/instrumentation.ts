...
export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    const { assertRlsSafe, bootstrapRls, ensureDeckOverridesSchema, ensureHouseStyleSchema, ensureReportsSchema, ensureLiveHighlightsSchema, ensureWorkspaceViewsSchema, ensureProjectsSchema, ensureWorkflowsSchema, ensurePrivateCompanySchema } = await import("@workspace/db");
    ...
    // Self-heal the `house_style` table before applying RLS. Same fail-fast
    // contract as deck_overrides — the table must exist by the time
    // `bootstrapRls()` runs or its tenant-isolation policy won't attach.
    try {
      await ensureHouseStyleSchema();
    } catch (e) {
      console.error("[instrumentation] failed to bootstrap house_style schema", e);
      throw e;
    }
    ...
    // Self-heal the Projects tables (projects, project_members,
    // project_activity, project_links) so RLS policies attach on first boot.
    try {
      await ensureProjectsSchema();
    } catch (e) {
      console.error("[instrumentation] failed to bootstrap projects schema", e);
      throw e;
    }

    // Self-heal the `workflows` / `workflow_runs` tables before RLS. Same
    // fail-fast contract: the visual workflow editor 500s without them, and
    // the RLS DO blocks guard on `IF EXISTS` so the tables must exist before
    // `bootstrapRls()` runs for their policies to attach on first boot.
    try {
      await ensureWorkflowsSchema();
    } catch (e) {
      console.error("[instrumentation] failed to bootstrap workflows schema", e);
      throw e;
    }

    // Self-heal the `reports` / `report_blocks` tables before RLS. Same
    // fail-fast contract: the report builder 500s without them, and the RLS DO
    // blocks guard on `IF EXISTS` so the tables must exist before
    // `bootstrapRls()` runs for their policies to attach on first boot.
    try {
      await ensureReportsSchema();
    } catch (e) {
      console.error("[instrumentation] failed to bootstrap reports schema", e);
      throw e;
    }

    // Bootstrap private_financials + private_cap_table before RLS so the
    // tenant-isolation policies can attach on first boot (the DO blocks guard
    // on IF EXISTS). DDL is idempotent.
    try {
      await ensurePrivateCompanySchema();
    } catch (e) {
      console.error("[instrumentation] failed to bootstrap private company schema", e);
      throw e;
    }
