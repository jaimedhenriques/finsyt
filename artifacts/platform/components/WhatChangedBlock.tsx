...
export default function WhatChangedBlock() {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.allSettled([
      fetch('/api/movers', { cache: 'no-store' }).then(r => r.ok ? r.json() : null),
      fetch('/api/news?limit=3', { cache: 'no-store' }).then(r => r.ok ? r.json() : null),
    ]).then(([m, n]) => {
      if (cancelled) return

      // Movers: combine top gainers + top losers, pick the 3 most extreme
      if (m.status === 'fulfilled' && m.value && (m.value.gainers?.length || m.value.losers?.length)) {
        const gainers: any[] = (m.value.gainers || []).slice(0, 3)
        const losers:  any[] = (m.value.losers  || []).slice(0, 3)
        const combined: MoverRow[] = [...gainers, ...losers].map((s: any) => ({
          symbol: String(s.symbol || ''),
          name:   String(s.name   || ''),
          pct:    Number(s.changePct) || 0,
        }))
        // Sort by abs changePct descending, keep top 4
        combined.sort((a, b) => Math.abs(b.pct) - Math.abs(a.pct))
        if (combined.length) {
          setMovers(combined.slice(0, 4))
        } else {
          setMoversError(true)
        }
      } else {
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ display:'flex', flexDirection:'column' }}>
          ...
          {!loading && movers.length === 0 && renderEmpty(
            moversError
              ? 'Market movers unavailable — configure a data provider to see live moves.'
              : 'No movers data yet.',
          )}
          {movers.map((m, i) => (
            <Link key={m.symbol} href={`/app/company/${m.symbol}`} style={{
