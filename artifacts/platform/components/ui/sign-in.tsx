...
export interface SignInPageProps {
  ...
  enterpriseSSO?: EnterpriseSSOOptions | null
}
            ...
            <div className="mt-3">
              {!showSSOForm ? (
                <button
                  ...
                  disabled={loading}
                  className="flex w-full items-center justify-center gap-3 rounded-md border border-[#D8DCE3] bg-white py-3.5 text-[15px] font-medium text-[#4A5366] transition-colors hover:bg-[#F4F7FB] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <BuildingIcon />
                  Enterprise SSO
                </button>
                ...
                <form
                  onSubmit={(e) => {
                    e.preventDefault()
                    if (ssoEmail) enterpriseSSO.onSubmit(ssoEmail)
                  }}
                  className="rounded-md border border-[#D8DCE3] bg-[#F8FAFC] p-4"
                >
                  <label className="text-[12px] font-medium uppercase tracking-wider text-[#6B7488]">
                    Work email for SSO
                  </label>
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      type="submit"
                      disabled={enterpriseSSO.submitting || !ssoEmail}
                      className="flex-1 rounded-md py-2.5 text-[14px] font-semibold text-white transition-colors disabled:cursor-not-allowed disabled:opacity-60"
                      style={{ background: ACCENT }}
                    >
                      {enterpriseSSO.submitting ? "Redirecting…" : "Continue with SSO →"}
                    </button>
                    <button
                      type="button"
                      onClick={() => { setShowSSOForm(false); setSSOEmail("") }}
                      className="rounded-md border border-[#D8DCE3] bg-white px-4 py-2.5 text-[14px] text-[#4A5366] hover:bg-[#F4F7FB]"
                    >
                      Cancel
                    </button>
                  </div>
