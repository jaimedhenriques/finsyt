'use client'
/**
 * FloatingCopilot — fixed bottom-right affordance present on every
 ...
 *                           the copilot affordance permanently visible
...
const PIN_KEY = 'finsyt:copilot-pinned'

export function FloatingCopilot({ onOpen, hint = 'Ask Finsyt' }: { onOpen: () => void; hint?: string }) {
  ...
  useEffect(() => {
    try { setPinned(localStorage.getItem(PIN_KEY) === '1') } catch { /* SSR / privacy mode */ }
  }, [])
  ...
  // Docked variant — slim vertical rail anchored to right edge centre. Stays
  // out of the way of content but is always visible while reading.
  if (pinned) {
    return (
      <div
        style={{
      ...
      >
        <button
          type="button"
          aria-label="Open Finsyt Copilot"
          title={`${hint} (⌘J)`}
          onClick={onOpen}
          style={{
  ...
  return (
    <div style={{ position: 'fixed', right: 22, bottom: 22, zIndex: 1050, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      <button
        type="button"
        aria-label="Pin Copilot to side rail"
        title="Pin Copilot"
        onClick={togglePin}
        style={{
