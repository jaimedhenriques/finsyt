...
interface ActivityItem { symbol: string; name: string; type: string; detail: string; ago: number; ts: string }
...
export default function ActivityFeed({ pollMs = 30_000, limit = 8 }: { pollMs?: number; limit?: number }) {
  ...
  return (
    <div style={{ borderRadius: 14, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div>
        {items.map((it, i) => (
          <Link key={i} href={`/app/company/${it.symbol}`}
            onClick={() => track('activity_feed_open', { symbol: it.symbol, type: it.type })}
