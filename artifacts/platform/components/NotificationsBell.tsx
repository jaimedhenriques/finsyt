...
interface LiveHighlightNotif {
  id: string
  kind: 'first_pin' | 'end_of_call' | 'workflow'
  symbol: string
  event: string
  callKey: string
  message: string
...
export default function NotificationsBell() {
  ...
  // Delegated analyst jobs.
  async function refreshJobs() {
    try {
      const r = await fetch(`${BASE}/api/agent-jobs`, { cache: 'no-store' })
      if (!r.ok) return
      const d = await r.json()
      const list: BellJob[] = Array.isArray(d?.jobs) ? d.jobs.slice(0, 8) : []
      setJobs(list)
      setUnreadJobCount(Number(d?.unreadCount ?? 0))
    } catch {}
  }

  useEffect(() => { setSeenNews(loadSeen()) }, [])
  useEffect(() => {
    refreshRuns(); refreshNews(); refreshLive(); refreshJobs()
    const id = setInterval(() => { refreshRuns(); refreshLive(); refreshJobs() }, 45_000)
    return () => clearInterval(id)
  }, [])

  // Refresh on open so the panel feels live.
  useEffect(() => { if (open) { refreshRuns(); refreshLive(); refreshJobs() } }, [open])
  ...
  const totalUnread = unreadRunCount + unreadNewsCount + unreadLiveCount + unreadJobCount
    ...
    <div ref={ref} style={{ position:'relative' }}>
      ...
      {open && (
        <div role="menu" style={{
                    ...
                    <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:11, fontWeight:700, color:'var(--accent-text)', marginBottom:2 }}>
                          {n.symbol} · {n.kind === 'workflow' ? 'Workflow' : n.kind === 'first_pin' ? 'First pin' : 'Call ended'}
                        </div>
          ...
          {/* Delegated jobs section */}
          {jobs.length > 0 && (
            <div>
              <SectionLabel>Delegated jobs · {unreadJobCount} unread</SectionLabel>
              {jobs.map(j => {
                const unread = !j.read
                const tone = j.status === 'failed' ? 'var(--danger, #f43f5e)' : 'var(--accent-text)'
                return (
                  <Link key={j.id}
                    ...
                    <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:11, fontWeight:700, color:tone, marginBottom:2, textTransform:'capitalize' }}>
                          {j.status} · {j.deliverableType.replace('_', ' ')}
                        </div>
                        <div style={{ fontSize:13, fontWeight:600, color:'var(--text-primary)', lineHeight:1.4 }}>{j.title}</div>
                      </div>
                      <span style={{ fontSize:10.5, color:'var(--text-muted)', flexShrink:0, whiteSpace:'nowrap' }}>{timeAgo(j.updatedAt)}</span>
                    </div>
                  </Link>
                )
              })}
