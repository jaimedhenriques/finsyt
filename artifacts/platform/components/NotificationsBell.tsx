...
export default function NotificationsBell() {
  ...
  useEffect(() => {
    refreshRuns(); refreshNews()
    const id = setInterval(refreshRuns, 45_000)
    return () => clearInterval(id)
  }, [])

  // Refresh on open so the panel feels live.
  useEffect(() => { if (open) refreshRuns() }, [open])
  ...
  const totalUnread = unreadRunCount + unreadNewsCount
    ...
    <div ref={ref} style={{ position:'relative' }}>
      ...
      {open && (
        <div role="menu" style={{
          ...
          {/* Empty state */}
          {!hasRuns && news.length === 0 && (
            <div style={{ padding:'24px 14px', textAlign:'center', color:'var(--text-muted)', fontSize:12.5 }}>
              You&apos;re all caught up.
            </div>
          )}

          <div style={{ padding:'10px 14px', textAlign:'center', borderTop: hasRuns || news.length ? '1px solid var(--border)' : 'none' }}>
