...
export type PeerCompareMetric = { key: string; label: string; demo: boolean; ntm?: boolean }
...
type Props = {
  ...
  csvBaseName?: string
  /** Fired when a peer row is clicked — used to drive the alt-data focus ticker. */
  onSelectSymbol?: (symbol: string) => void
  /** Symbol currently driving the alt-data cards; its row is highlighted. */
...
export default function PeerSelectedTable({
  ...
  const metricsMeta: PeerCompareMetric[] = data?.metricsMeta ?? metricKeys.map((k) => ({ key: k, label: k, demo: false, ntm: false }))
        ...
        <table className="data-table" style={{ width: '100%', minWidth: 880 }}>
          ...
          <tbody>
            ...
            {!loading && rows.map((r, i) => {
              ...
              const isActive = !!activeSymbol && r.symbol === activeSymbol
              const bg = isActive ? 'var(--accent-bg, rgba(99,102,241,0.12))' : isAnchor ? 'rgba(99, 102, 241, 0.06)' : undefined
              return (
                <tr
                  key={r.symbol}
                  onClick={onSelectSymbol ? () => onSelectSymbol(r.symbol) : undefined}
                  aria-selected={onSelectSymbol ? isActive : undefined}
                  style={{ ...(bg ? { background: bg } : {}), ...(onSelectSymbol ? { cursor: 'pointer' } : {}) }}
                >
