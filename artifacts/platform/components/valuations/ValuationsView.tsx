...
export default function ValuationsView({ symbol, initialQuote, initialPeers, className }: ValuationsViewProps) {
  ...
  const [enabled, setEnabled] = useState<Record<string, boolean>>({
    ...
    'peer:TEV/EBIT': true,
    'tx:Implied EV / Revenue': true,
    'tx:Implied EV / EBITDA': true,
    'tx:Implied EV / EBIT': true,
    'tx:Implied Equity Value / Net Income': true,
    'dcf': true,
  })
  ...
  const { bands, currentPrice, weightedValuation, weightedFrom, loading, quote, txComps } = result
  ...
  const toggleKeys: Array<{ key: string; label: string }> = useMemo(() => ([
    ...
    ...TX_ROW_LABELS.map(l => ({ key: `tx:${l}`, label: `TX · ${l.replace('Implied ', '').replace('Equity Value', 'Equity')}` })),
    { key: 'dcf',                  label: 'DCF' },
  ]), [])
                ...
                <>
                  ...
                  <div role="dialog" aria-label="Weighted valuation calculation"
                    ...
                    <ul style={{ margin: '4px 0 0', padding: '0 0 0 16px', color: 'var(--text-secondary)', fontSize: 11.5, lineHeight: 1.5 }}>
                      <li><b>TEV/Revenue (Peer)</b> uses Price/Sales as a proxy because the upstream provider does not expose a clean TEV/Revenue field. Materially levered peers may distort the band.</li>
                      <li><b>TEV/EBIT (Peer)</b> is not exposed by the data provider and renders as a placeholder row.</li>
                      <li><b>Transaction Comps</b> are sourced from FMP M&A filings. Deal EV is the filed <em>totalDealValue</em>; target financials are the annual period closest to the deal date. EV multiples are bridged to equity via the subject's most recent net debt. Equity/NI treats the deal value as a proxy for equity consideration (conservative when deals include assumed debt). Q1–Q3 range requires ≥ 2 comparables with usable financials.{txComps?.compCount != null && txComps.compCount > 0 ? ` ${txComps.compCount} comparable deal${txComps.compCount === 1 ? '' : 's'} found.` : ''}</li>
                    </ul>
                  </div>
                </>
              )}
