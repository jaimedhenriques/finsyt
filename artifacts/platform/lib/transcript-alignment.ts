...
// Read just the raw STT word stream cached for this call. Use this for live
// surfaces (CMDs, conferences, anything without a canonical paragraph layout)
// where you want to emit captions directly off the STT timeline rather than
// going through paragraph alignment.
export interface CachedSttWord { word: string; startSec: number; endSec: number }
export function getCachedStt(cacheKey: string): CachedSttWord[] | null {
  let entry = cache.get(cacheKey)
  if (!entry || Date.now() - entry.ts > CACHE_TTL_MS) {
    const disk = diskReadSync(cacheKey)
    if (disk) {
  ...
  if (!entry || !entry.stt) return null
  if (Date.now() - entry.ts > CACHE_TTL_MS) return null
  return entry.stt.map(w => ({ word: w.word, startSec: w.start, endSec: w.end }))
}
