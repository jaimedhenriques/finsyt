...
import { runWithApiKeyEntitlement } from "./api-entitlement-context";
...
export function withPublicApi(
  ...
  return async (req: NextRequest): Promise<NextResponse> => {
    ...
    let status = 500;
    try {
      // Activate the API-key entitlement context so internally-invoked route
      // handlers (transcripts/insider/ownership gates) resolve the caller's
      // plan from the key's org instead of an absent Clerk session.
      response = await runWithApiKeyEntitlement(auth.key, () =>
        handler(req, { key: auth.key, endpoint: opts.endpoint }),
      );
