...
export async function assemblePeerComparison(
  ...
  if (hasRealForwardPe) {
    dataSources.push({ name: 'FMP analyst estimates', category: 'provider', detail: 'Forward consensus EPS → forward P/E' })
  }
  if (hasDemoCells) {
    dataSources.push({ name: 'Finsyt estimate model', category: 'model', detail: 'Illustrative demo cells (NTM EV/EBITDA, % options ITM, fallback forward P/E)' })
  }
