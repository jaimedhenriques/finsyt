...
import {
  ...
  type PeerComparisonColumn,
  type PeerComparisonBodyRow,
  type PeerComparisonSummaryRow,
} from './deck-service'
...
export interface PeerComparisonDeckInput {
  /** Peer-set name rendered as the deck title (e.g. "Mega-Cap Tech"). */
  setName?:    string
  /** Anchor / subject ticker pinned at the top of the comparison table. */
  subject?:    string
  asOf:        string
  footerLine?: string

  /** Top-of-deck narrative bullets for the overview slide. */
  overviewBullets?: string[]
  /** Median / mean valuation-context KPI tiles. */
  valuationTiles?:  { label: string; value: string }[]
...
export function peerComparisonTemplate(input: PeerComparisonDeckInput): DeckTemplate {
  const setName = input.setName?.trim() || 'Peer Comparison'
  const title = input.subject ? `${input.subject} · ${setName}` : setName
  const sections: DeckSection[] = []

  // 1) Overview narrative
  if (input.overviewBullets?.length) {
    sections.push({
      type: 'executive-summary',
      data: { title: 'Overview', bullets: input.overviewBullets },
      citations: input.citations,
    })
  }

  // 2) Median valuation context tiles
  ...
  return {
    ...
    context: {
      brand: FINSYT_BRAND,
      cover: {
        eyebrow:  'Finsyt Peer Comparison',
        title,
        subtitle: `${input.rows.length} compan${input.rows.length === 1 ? 'y' : 'ies'}`,
        asOf:     input.asOf,
        presenter: 'Finsyt Research',
      },
      asOf:        input.asOf,
      footerLine:  input.footerLine || 'Sources: Finsyt platform data.',
      identityLine: setName,
      dataSources: input.dataSources || [
        { name: 'Finsyt platform data', category: 'provider', detail: 'Quote-derived comparables' },
      ],
      globalCitations: input.citations,
    },
    sections,
    meta: { title: `${setName} — Peer Comparison`, subject: `Generated ${input.asOf}` },
  }
}
