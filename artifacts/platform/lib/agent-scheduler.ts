...
import { tickFilingSignalWatcher } from './filing-signal-watcher'
import { tickWorkflowScheduler } from './workflows/scheduler'
...
// Filing-signal watcher cadence — every 15 minutes. The SEC EDGAR
// Intelligence scoring call is a paid Apify run per (org, ticker), so we
// poll less aggressively than the free-API rerun watcher.
const FILING_SIGNAL_WATCHER_CRON = '*/15 * * * *'
...
let filingSignalWatcherTask: any = null
let filingSignalWatcherInFlight = false
...
export function startAgentScheduler() {
  ...
  // Visual workflow scheduler — same per-minute cron, separate handle +
  // in-flight guard. Boot kick a few seconds after the agent tick so the two
  // cross-org scans don't fire on the exact same instant on startup.
  workflowTask = cron.schedule(TICK_CRON, () => {
    workflowTick().catch((e) => console.error('[workflow-scheduler] tick error', (e as Error).message))
  })
  setTimeout(() => { workflowTick().catch(() => {}) }, 8_000)
  ...
  // Filing-signal watcher: every 15 minutes, score recent SEC filings for
  // every watchlisted ticker (per-org Apify connection) and pin a Live
  // Highlight + fire an alert for any filing scoring at/above the org's
  // threshold. Gated by env so hosts without an Apify connection (or
  // perf-sensitive deployments) can opt out independently.
  if (process.env.FILING_SIGNAL_WATCHER_DISABLED !== '1') {
    filingSignalWatcherTask = cron.schedule(FILING_SIGNAL_WATCHER_CRON, () => {
      runFilingSignalWatcherTick().catch((e) =>
        console.error('[filing-signal-watcher] tick error', (e as Error).message),
      )
    })
    // Boot kick — surface filings that landed while the server was offline
    // without waiting the full 15-minute interval.
    setTimeout(() => { runFilingSignalWatcherTick().catch(() => {}) }, 20_000)
  }
...
async function runFilingSignalWatcherTick() {
  if (filingSignalWatcherInFlight) return
  filingSignalWatcherInFlight = true
  try {
    const res = await tickFilingSignalWatcher()
    if (res.pinned > 0) {
      console.log(
        `[filing-signal-watcher] scanned ${res.scannedOrgs} orgs, ${res.scannedSymbols} symbols, pinned ${res.pinned}/${res.qualifyingFilings} high-signal filings`,
      )
    }
  } finally {
    filingSignalWatcherInFlight = false
  }
}

export function stopAgentScheduler() {
  if (task) { try { task.stop() } catch {} }
  if (workflowTask) { try { workflowTask.stop() } catch {} }
  if (credentialNotifierTask) { try { credentialNotifierTask.stop() } catch {} }
  if (filingWatcherTask) { try { filingWatcherTask.stop() } catch {} }
  if (filingSignalWatcherTask) { try { filingSignalWatcherTask.stop() } catch {} }
  if (liveHighlightsCleanupTask) { try { liveHighlightsCleanupTask.stop() } catch {} }
  task = null
  workflowTask = null
  workflowTickInFlight = false
  credentialNotifierTask = null
  filingWatcherTask = null
  filingWatcherInFlight = false
  filingSignalWatcherTask = null
  filingSignalWatcherInFlight = false
...
// Workflow scheduler tick: piggy-backs on the same per-minute cron. Kept on its
// own in-flight guard + cron handle so a slow workflow run can't block the
// agent tick (and vice-versa), and so dev hot-reload stop→start can tear it
// down cleanly.
let workflowTickInFlight = false
async function workflowTick() {
  if (workflowTickInFlight) return
  workflowTickInFlight = true
  try {
    const res = await tickWorkflowScheduler(SCAN_LIMIT)
    if (res.due > 0) {
      console.log(`[workflow-scheduler] ${res.due} due — ${res.ok} ok, ${res.failed} failed`)
    }
  } finally {
    workflowTickInFlight = false
  }
