...
// ── Widget catalogue ─────────────────────────────────────────────────────────
// Every catalogue entry maps to a live-data widget component in
// `components/widgets/index.tsx`. `defaultW` is the column span (1-4 of a
// four-column grid) a freshly-added widget gets; the user can resize it in
// edit mode. `chromeless` widgets render their own card/section styling so the
// grid skips the standard widget header (used for self-contained blocks like
// the Ask bar, "What changed", Live now and the agents feed).
export type WidgetCategory = 'Markets' | 'Watchlist' | 'News' | 'AI & Agents' | 'Workspace'
...
export interface WidgetDef {
  ...
  defaultW: number       // grid column span 1..4
  chromeless?: boolean   // render without the standard widget header card
}

export const WIDGET_CATALOGUE: WidgetDef[] = [
  // AI & Agents
  { id:'ai_quick_ask',       label:'Ask Finsyt',          description:'Page-aware agent query bar for your morning brief', icon:'🤖', category:'AI & Agents', defaultW:4, chromeless:true },
  { id:'what_changed',       label:'What Changed',        description:'New filings, news and price moves since your last visit', icon:'✨', category:'AI & Agents', defaultW:4, chromeless:true },
  { id:'agents_feed',        label:'Workflow Agents',     description:'Pre-built and custom agents monitoring your universe', icon:'⚡', category:'AI & Agents', defaultW:3, chromeless:true },
  { id:'activity_feed',      label:'Activity Feed',       description:'Recent agent runs and research activity', icon:'📋', category:'AI & Agents', defaultW:1, chromeless:true },
  // Markets
  { id:'indices',            label:'Live Indices',        description:'S&P 500, Nasdaq, Dow, Russell, Treasuries, Gold', icon:'📈', category:'Markets', defaultW:4, chromeless:true },
  { id:'top_movers',         label:'Market Movers',       description:'Top gainers, losers and most active names', icon:'🔄', category:'Markets', defaultW:2 },
  { id:'macro_snapshot',     label:'Macro Snapshot',      description:'Fed funds, CPI, GDP, unemployment & yields', icon:'🌍', category:'Markets', defaultW:2 },
  { id:'live_now',           label:'Live Now',            description:'Markets open status and live event strip', icon:'🔴', category:'Markets', defaultW:3, chromeless:true },
  // Watchlist
  { id:'watchlist_coverage', label:'Intelligence Coverage', description:'Live price + spark cards across your watchlist', icon:'⭐', category:'Watchlist', defaultW:4, chromeless:true },
  { id:'watchlist_rail',     label:'Watchlist Snapshot',  description:'Compact list of your tracked tickers', icon:'📊', category:'Watchlist', defaultW:1, chromeless:true },
  { id:'price_monitor',      label:'Price Monitor',       description:'Real-time price ticker for your watchlist', icon:'💹', category:'Watchlist', defaultW:2 },
  { id:'portfolio_summary',  label:'Portfolio Summary',   description:'Your synced positions and day P&L', icon:'💼', category:'Watchlist', defaultW:2 },
  // News
  { id:'news_feed',          label:'Latest Headlines',    description:'Real-time news with sentiment tags', icon:'📰', category:'News', defaultW:2, chromeless:true },
  { id:'earnings_ahead',     label:'Earnings Ahead',      description:'Upcoming earnings across your coverage', icon:'🗓', category:'News', defaultW:2, chromeless:true },
  // Workspace
  { id:'connect_sources',    label:'Connect Sources',     description:'Wire your data providers into the workspace', icon:'🔗', category:'Workspace', defaultW:4, chromeless:true },
  { id:'quick_access',       label:'Quick Access',        description:'Jump to research, models, screener and more', icon:'🚀', category:'Workspace', defaultW:4, chromeless:true },
]
...
export interface PlacedWidget {
  ...
  order: number
  w?: number          // column span 1..4 (defaults to the catalogue defaultW)
  config?: Record<string, unknown>
}

export type PageLayouts = Record<string, PlacedWidget[]>

// The default Overview board reproduces today's Overview page content exactly,
// in the same order, so brand-new users (and anyone who has never customised)
// see the identical morning brief. Each entry is a live-data widget.
const DEFAULT_HOME_LAYOUT: PlacedWidget[] = [
  { id:'w-ai-quick-ask',   widgetId:'ai_quick_ask',       order:0, w:4 },
  { id:'w-what-changed',   widgetId:'what_changed',       order:1, w:4 },
  { id:'w-connect',        widgetId:'connect_sources',    order:2, w:4 },
  { id:'w-live-now',       widgetId:'live_now',           order:3, w:3 },
  { id:'w-activity',       widgetId:'activity_feed',      order:4, w:1 },
  { id:'w-indices',        widgetId:'indices',            order:5, w:4 },
  { id:'w-coverage',       widgetId:'watchlist_coverage', order:6, w:4 },
  { id:'w-agents',         widgetId:'agents_feed',        order:7, w:3 },
  { id:'w-rail',           widgetId:'watchlist_rail',     order:8, w:1 },
  { id:'w-news',           widgetId:'news_feed',          order:9, w:2 },
  { id:'w-earnings',       widgetId:'earnings_ahead',     order:10, w:2 },
  { id:'w-quick-access',   widgetId:'quick_access',       order:11, w:4 },
]
...
interface WorkspaceCtx {
  ...
  resizeWidget: (page: string, instanceId: string, w: number) => void
  /**
   * `true` when the active page layout syncs to the workspace via the API
   * (signed-in user with an active org). `false` for signed-out users and
   * signed-in users without an org, where layouts stay in localStorage on
   * this browser only.
   */
  layoutsSynced: boolean
  // Screener presets
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const resizeWidget = useCallback((page: string, instanceId: string, w: number) => {
    const clamped = Math.max(1, Math.min(4, Math.round(w)))
    setLayouts(prev => {
      const next = {
        ...prev,
        [page]: (prev[page] || []).map(x => (x.id === instanceId ? { ...x, w: clamped } : x)),
      }
      localStorage.setItem('finsyt-layouts', JSON.stringify(next))
      return next
    })
  ...
  // ── Layout persistence to the workspace ──────────────────────────────────
  // The Overview board syncs per-user + per-org via /api/dashboard-layout.
  // localStorage stays the offline-first cache (and sole store for signed-out
  // users); for signed-in org members the API is the source of truth. We only
  // persist after hydration so the initial GET never bounces straight back as a
  // redundant PUT, and we skip writes whose payload matches what we last saved.
  const layoutsSynced = Boolean(authLoaded && isSignedIn && orgId)
  const SYNCED_PAGE = '/app'
  const layoutHydratedRef = useRef(false)
  const lastPersistedRef = useRef<string>('')

  useEffect(() => {
    if (!layoutsSynced) { layoutHydratedRef.current = false; return }
    let cancelled = false
    fetch(`/api/dashboard-layout?page=${encodeURIComponent(SYNCED_PAGE)}`, { credentials: 'same-origin' })
      .then(r => r.ok ? r.json() : Promise.reject(new Error(`HTTP ${r.status}`)))
      .then((data: { layout?: { widgets?: PlacedWidget[] } | null }) => {
        if (cancelled) return
        const widgets = data.layout?.widgets
        if (Array.isArray(widgets) && widgets.length > 0) {
          lastPersistedRef.current = JSON.stringify(widgets)
          setLayouts(prev => {
            const next = { ...prev, [SYNCED_PAGE]: widgets }
            try { localStorage.setItem('finsyt-layouts', JSON.stringify(next)) } catch {}
            return next
          })
        } else {
          // No saved board yet → keep the default Overview and treat it as the
          // baseline so the first edit persists but the untouched default does not.
          lastPersistedRef.current = ''
        }
      })
      .catch(() => { /* keep last-known cache on transient failure */ })
      .finally(() => { if (!cancelled) layoutHydratedRef.current = true })
    return () => { cancelled = true }
  }, [layoutsSynced, orgId])

  useEffect(() => {
    if (!layoutsSynced || !layoutHydratedRef.current) return
    const widgets = layouts[SYNCED_PAGE] || []
    ...
    const t = setTimeout(() => {
      fetch('/api/dashboard-layout', {
        ...
        headers: { 'content-type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({ page: SYNCED_PAGE, widgets }),
      })
        .then(r => { if (r.ok) lastPersistedRef.current = json })
        .catch(() => { /* localStorage already holds the latest value */ })
    }, 700)
    return () => clearTimeout(t)
  }, [layouts, layoutsSynced])
  ...
  return (
    <Ctx.Provider value={{ nav, setNav, topbar, setTopbar, layouts, setLayout, editMode, setEditMode, pickerOpen, pickerPage, openPicker, closePicker, addWidget, removeWidget, reorderWidgets, resizeWidget, layoutsSynced, screenerPresets, screenerPresetsSynced, saveScreenerPreset, deleteScreenerPreset, renameScreenerPreset, updateScreenerPreset, theme, setTheme, dataSourcesFooterEnabled, setDataSourcesFooterEnabled, dataSourcesFooterCollapsed, setDataSourcesFooterCollapsed }}>
      {children}
    </Ctx.Provider>
  )
}
