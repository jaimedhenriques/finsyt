...
const DATA_TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      "Search the tenant's private Research Library (arXiv papers, finance reports, quant research). Returns the most relevant passages with citations. Use when the user asks about academic research, papers, quant models, or any topic where they may have ingested reference material.",
    parameters: {
      type: "object",
      properties: {
        query: { type: "string", description: "The search query — a question or key concept to find relevant research for." },
        limit: { type: "number", description: "Max results (1–12). Defaults to 6." },
      },
      required: ["query"],
    },
    run: async (a, ctx) => {
      ...
      const limit = Math.min(Number(a.limit) || 6, 12);
      const d: any = await dataFetch(
        ctx,
        `/research-library/search?${new URLSearchParams({ q, limit: String(limit) })}`,
      );
      if (!d || d.error) return { results: [], note: d?.error || "No research library data." };
      if (!d.results?.length) return { results: [], note: `No results found in the Research Library for: ${q}`, totalPapers: d.totalPapers || 0 };
      return {
        results: trim(d.results || [], limit).map((r: any) => ({
          title: r.title,
          snippet: r.snippet,
