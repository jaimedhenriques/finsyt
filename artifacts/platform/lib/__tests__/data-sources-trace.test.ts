...
test('providerKeyFromSource: maps premium BYO-license connector labels to provider keys', () => {
  // These labels are emitted by the agent route's premiumConnectorLabel()
  // helper whenever a FactSet/CapIQ/etc. tool result is sent as a
  // tool_result SSE frame. The assertion confirms the round-trip:
  //   label → providerKeyFromSource → PROVIDER_META key
  assert.equal(providerKeyFromSource('FactSet (your license)'), 'factset')
  assert.equal(providerKeyFromSource('S&P Capital IQ (your license)'), 'capiq')
  assert.equal(providerKeyFromSource('Refinitiv / LSEG (your license)'), 'refinitiv')
  assert.equal(providerKeyFromSource('Bloomberg Data License (your license)'), 'bloomberg')
  assert.equal(providerKeyFromSource('PitchBook (your license)'), 'pitchbook')
  // Also match raw source strings that may arrive from the executor
  assert.equal(providerKeyFromSource('FactSet'), 'factset')
  assert.equal(providerKeyFromSource('Capital IQ Pro'), 'capiq')
  assert.equal(providerKeyFromSource('S&P Capital'), 'capiq')
  assert.equal(providerKeyFromSource('Refinitiv Data Platform'), 'refinitiv')
  assert.equal(providerKeyFromSource('LSEG symbology'), 'refinitiv')
  assert.equal(providerKeyFromSource('Bloomberg BEAP'), 'bloomberg')
  assert.equal(providerKeyFromSource('PitchBook Data'), 'pitchbook')
})
