...
export interface AgentTool {
  ...
  /**
   * Catalog slug for the connector definition this tool came from (e.g.
   * "factset", "spglobal-capiq"). Present only for catalog-backed connections;
   * absent for custom REST / MCP connections without a definition. Used by the
   * agent route to produce a "FactSet (your license)" provider label in the
   * `tool_result` SSE frame so the citation/trace pipeline can attribute the
   * result to the correct premium BYO-license connector.
   */
  _catalogSlug?: string;
  /**
   * Whether this connector is a premium BYO-license provider (FactSet,
   * Capital IQ, etc.). Derived server-side from the catalog entry so the
   * agent can produce a "your license" attribution label without a round-trip.
   */
  _isPremium?: boolean;
}
  ...
  return withOrgContext(orgId, async (tx) => {
    const rows = await tx
      .select({
        ...
        operationHidden: connectionOperationsTable.hidden,
        paramSchema: connectionOperationsTable.paramSchema,
        // Join definitions to get the catalog slug so premium provider
        // attribution ("FactSet (your license)") can be produced in the
        // agent route without a second DB query or a catalog table scan.
    ...
    return rows
      // `operationHidden` rows are credential-validation probes (e.g.
      // Apify's users_me) — the executor can still call them by name, but
      // they must not surface as agent / MCP tools.
      .filter((r) => r.operationEnabled && !r.operationHidden && r.connectionStatus === "active")
      .map<AgentTool>((r) => {
        ...
        const catalogSlug = r.definitionSlug ?? undefined;
        const isPremium = catalogSlug ? !!(findCatalogEntry(catalogSlug)?.isPremium) : false;
        return {
          name,
          description: isPremium
            ? `${r.operationDescription || r.operationName} (via "${r.connectionName}" — your license)`
            : `${r.operationDescription || r.operationName} (via "${r.connectionName}" connection)`,
