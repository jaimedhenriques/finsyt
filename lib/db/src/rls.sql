...
-- `SET LOCAL ROLE app_runtime` (issued inside withOrgContext /
-- withClerkContext) is permitted. Without this, deployments whose
-- connection role has BYPASSRLS (e.g. Neon's neondb_owner) fail with
-- "permission denied to set role app_runtime" on every tenant query.
-- Idempotent: skipped when the grant already exists.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
DECLARE
  t text;
  tables text[] := ARRAY['leads', 'research_notes', 'chat_messages', 'memberships', 'portfolio_positions', 'api_keys', 'alerts', 'workspaces', 'workspace_views', 'matrices', 'matrix_snapshots', 'financial_templates', 'private_financials', 'private_cap_table'];
BEGIN
...
-- ── User preferences ────────────────────────────────────────────────────────
-- `user_preferences` is per-user, not per-org: each row carries the analyst's
-- agent-transparency toggles so they follow the user across browsers, devices
-- and even between workspaces. RLS keys on `app.current_clerk_user_id`
-- (set by `withClerkUserContext` in lib/db/src/index.ts) so a forgotten WHERE
-- clause cannot leak another user's preferences.
...
-- ── Routing policies ────────────────────────────────────────────────────────
-- `routing_policies` is per-org: each row stores the admin-configured source
-- priority list for one data domain. RLS keys on `app.current_org_id`
-- (local UUID) matching the pattern used by all other connector tables.

ALTER TABLE routing_policies ENABLE ROW LEVEL SECURITY;
ALTER TABLE routing_policies FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS routing_policies_select ON routing_policies;
CREATE POLICY routing_policies_select ON routing_policies
  FOR SELECT
  USING (org_id::text = current_setting('app.current_org_id', true));

DROP POLICY IF EXISTS routing_policies_insert ON routing_policies;
CREATE POLICY routing_policies_insert ON routing_policies
  FOR INSERT
  WITH CHECK (org_id::text = current_setting('app.current_org_id', true));

DROP POLICY IF EXISTS routing_policies_update ON routing_policies;
CREATE POLICY routing_policies_update ON routing_policies
  FOR UPDATE
  USING (org_id::text = current_setting('app.current_org_id', true))
  WITH CHECK (org_id::text = current_setting('app.current_org_id', true));

DROP POLICY IF EXISTS routing_policies_delete ON routing_policies;
CREATE POLICY routing_policies_delete ON routing_policies
  FOR DELETE
  USING (org_id::text = current_setting('app.current_org_id', true));
