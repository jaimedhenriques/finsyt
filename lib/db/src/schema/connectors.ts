...
export const connectionOperationsTable = pgTable(
  "connection_operations",
  {
    ...
    /**
     * Internal-only operations (hidden from the agent / MCP tool registry).
     * Still callable through the executor by name — used for credential-
     * validation probes (e.g. Apify's /users/me) that must run on connect
     * but should not be exposed as a fourth tool to the agent.
     */
...
// ── routing_policies ────────────────────────────────────────────────────────
// Per-org, per-domain source-priority list. When a tenant has multiple
// federated connections covering the same domain (e.g. FactSet + CapIQ +
// built-in FMP for fundamentals), the router tries sources in the order
// defined here, falling over on failure/empty, and attaches attribution.
//
// `domain` values mirror the catalog categories + agent tool domains:
//   quotes | fundamentals | news | filings | estimates |
//   transcripts | macro | ownership | deals
//
// `sources` is a JSONB array of PolicySource objects (ordered, highest-priority first):
//   { type: 'connection' | 'builtin', id: string, label: string,
//     catalogSlug?: string, operationName?: string, disabled?: boolean }
export const routingPoliciesTable = pgTable(
  "routing_policies",
  {
    ...
    orgId: uuid("org_id")
      .notNull()
      .references(() => organizationsTable.id, { onDelete: "cascade" }),
    /** Data domain this policy applies to. */
    domain: text("domain").notNull(),
    /**
     * Ordered list of candidate sources (highest priority first). Each entry:
     *   type        — 'connection' | 'builtin'
     *   id          — connection UUID or builtin slug (e.g. 'fmp')
     *   label       — display name
     *   catalogSlug — catalog slug for connector-backed connections
     *   operationName — which operation to invoke for this domain
     *   disabled    — when true the source is skipped without removal
     */
    sources: jsonb("sources").notNull().default("[]"),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => ({
    byOrg: index("routing_policies_org_idx").on(t.orgId),
    uniqOrgDomain: uniqueIndex("routing_policies_org_domain_uniq").on(t.orgId, t.domain),
  }),
);

// ── source_field_mappings ────────────────────────────────────────────────────
// Per-org, per-connection, per-domain field mapping for the BYOS engine.
// When a customer connects their own data source (Snowflake, FactSet, etc.)
// and selects it as the authoritative source for a domain, this table stores
// how that source's raw fields map to Finsyt's canonical datapoints.
//
// `field_map` is a JSONB object: { sourceField: canonicalKey, ... }
//   e.g. { "net_revenue": "revenue", "diluted_eps": "eps" }
// `coverage`  is a JSONB array of canonical keys that are mapped (covered).
export const sourceFieldMappingsTable = pgTable(
  "source_field_mappings",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    orgId: uuid("org_id")
    ...
    connectionId: uuid("connection_id")
      .notNull()
      .references(() => connectionsTable.id, { onDelete: "cascade" }),
    /** Data domain this mapping applies to. */
    domain: text("domain").notNull(),
    /**
     * Source field → canonical datapoint key map.
     * e.g. { "net_revenue": "revenue", "diluted_eps": "eps" }
     */
    fieldMap: jsonb("field_map").notNull().default("{}"),
    /**
     * Array of canonical keys that are covered by this mapping.
     * Derived from fieldMap values; stored for fast coverage queries.
     */
    coverage: jsonb("coverage").notNull().default("[]"),
    /** When the source was last introspected for available fields. */
    introspectedAt: timestamp("introspected_at", { withTimezone: true }),
    /** When the mapping was last reviewed and confirmed by the data team. */
    confirmedAt: timestamp("confirmed_at", { withTimezone: true }),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => ({
    byOrg: index("source_field_mappings_org_idx").on(t.orgId),
    byConn: index("source_field_mappings_conn_idx").on(t.connectionId),
    uniqOrgConnDomain: uniqueIndex("source_field_mappings_org_conn_domain_uniq").on(
      t.orgId,
      t.connectionId,
      t.domain,
    ),
...
// ── Types & Zod ─────────────────────────────────────────────────────────────
export type SourceFieldMapping = typeof sourceFieldMappingsTable.$inferSelect;
...
export type RoutingPolicy = typeof routingPoliciesTable.$inferSelect;
