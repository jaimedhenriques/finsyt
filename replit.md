...
External agents that open PRs on the GitHub mirror can be pulled back into the Replit workspace with `scripts/pull-from-github.sh`. It reuses the same credential helper as the outbound sync — no new secrets are needed.
...
**By PR number** (resolves the head branch via the public GitHub API):
```bash
bash scripts/pull-from-github.sh --pr 42
```

The script creates a local branch `github/<branch>` and prints a commit summary plus step-by-step instructions for reviewing and merging:

1. `git log main..github/<branch> --oneline` — review commits
2. `git diff main..github/<branch>` — review changes
3. `git checkout github/<branch>` — test locally
4. `pnpm run typecheck && pnpm run lint` — validate
5. `git merge --no-ff github/<branch>` into `main` — merge

After the merge the normal `post-merge.sh` run will mirror the result back to GitHub.