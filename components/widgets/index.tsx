...
// ── Colour helpers ────────────────────────────────────────────────────────────
const CC: Record<string,string> = { TSLA:'#CC0000',MSFT:'#00A4EF',NVDA:'#76B900',AAPL:'#555',META:'#0866FF',AMZN:'#FF9900',JPM:'#003087',GS:'#6DB33F',FED:'#1B4FFF',INTC:'#0068B5',AMD:'#ED1C24',TSM:'#EE1C25',AVGO:'#CA2026' }
const cBg = (s:string) => CC[s] ?? '#1B4FFF'
  ...
  return (
    <div style={{ padding:'10px 14px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
        ...
        <span style={{ fontSize:11, color:'#B0BCD0' }}>· {LIVE_E.length} companies</span>
      ...
      {IDXS.map((idx,i) => {
        ...
        return (
          <div key={idx.name} style={{ flex:'0 0 auto', minWidth:150, padding:'6px 16px', borderRight:i<IDXS.length-1?'1px solid #E8EDF4':'none' }}>
            ...
            <div style={{ fontSize:16, fontWeight:900, color:'#0A1628', letterSpacing:'-0.02em' }}>{idx.val.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontSize:11, fontWeight:700, color:pos?'#059669':'#DC2626' }}>{pos?'+':''}{idx.chg.toFixed(2)}%</span>
              <div style={{ width:60, height:24 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data} margin={{top:2,right:0,bottom:2,left:0}}>
                    <defs><linearGradient id={`ig${i}`} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={pos?'#059669':'#DC2626'} stopOpacity={0.2}/><stop offset="95%" stopColor={pos?'#059669':'#DC2626'} stopOpacity={0}/></linearGradient></defs>
                    <Area type="monotone" dataKey="v" stroke={pos?'#059669':'#DC2626'} strokeWidth={1.5} fill={`url(#ig${i})`} dot={false}/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
    ...
    <div style={{ padding:'10px 14px' }}>
      <div style={{ display:'flex', gap:12, marginBottom:10, borderBottom:'1px solid #F0F4FA', paddingBottom:8 }}>
        {(['active','gain','loss'] as const).map(k=>(
          <button key={k} onClick={()=>setTab(k)} style={{ fontSize:11, fontWeight:600, background:'none', border:'none', cursor:'pointer', fontFamily:'inherit', color:tab===k?'#0A1628':'#7D8FA9', borderBottom:`2px solid ${tab===k?'#DC2626':'transparent'}`, paddingBottom:4, paddingLeft:0, paddingRight:0, whiteSpace:'nowrap' }}>
      ...
      {MOVERS.map((m,i)=>(
        <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'7px 0', borderBottom:i<MOVERS.length-1?'1px solid #F5F7FB':'none' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <div>
              <div style={{ fontSize:11.5, fontWeight:700, color:'#0A1628' }}>{m.ticker}</div>
              <div style={{ fontSize:10, color:'#B0BCD0', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:100 }}>{m.name}</div>
            </div>
          </div>
          <div style={{ textAlign:'right' }}>
            <div style={{ fontSize:12, fontWeight:800, color:'#0A1628' }}>${m.price.toFixed(2)}</div>
            <div style={{ fontSize:11, fontWeight:700, color:m.chg>=0?'#059669':'#DC2626' }}>{m.chg>=0?'▲':'▼'} {Math.abs(m.chg).toFixed(2)}%</div>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ padding:'12px 16px' }}>
      <p style={{ fontSize:13, color:'#3D4F6E', marginBottom:10 }}>
        <strong style={{ color:'#059669' }}>{beat}%</strong> of S&P 500 companies <strong style={{ color:'#059669' }}>beat</strong> estimates this quarter.
      </p>
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        <div style={{ textAlign:'center' }}>
          <div style={{ fontSize:18, marginBottom:2 }}>🏆</div>
          <div style={{ fontSize:16, fontWeight:900, color:'#059669' }}>{bc}</div>
          <div style={{ fontSize:10, color:'#059669', fontWeight:600 }}>Beat</div>
        </div>
        <div style={{ flex:1, height:12, background:'#FEF2F2', borderRadius:999, overflow:'hidden' }}>
        ...
        <div style={{ textAlign:'center' }}>
          ...
          <div style={{ fontSize:16, fontWeight:900, color:'#DC2626' }}>{mc}</div>
      ...
      {SECTORS.map(s=>{
        ...
        return (
          <div key={s.name} style={{ display:'flex', alignItems:'center', gap:6 }}>
            ...
            <div style={{ flex:1, height:8, background:'#F0F4FA', borderRadius:999, overflow:'hidden' }}>
              <div style={{ height:'100%', width:`${(Math.abs(s.chg)/max)*100}%`, background:pos?'#6EE7B7':'#FCA5A5', borderRadius:999 }} />
            </div>
            <span style={{ fontSize:10, fontWeight:700, color:pos?'#059669':'#DC2626', width:44, textAlign:'right', flexShrink:0 }}>{pos?'+':''}{s.chg.toFixed(2)}%</span>
          </div>
        )
      })}
    </div>
...
export function LiveNewsFeedWidget() {
  const sb=(s:string)=>s==='bullish'?{bg:'#ECFDF5',c:'#059669',l:'Bullish'}:s==='bearish'?{bg:'#FEF2F2',c:'#DC2626',l:'Bearish'}:{bg:'#F5F7FB',c:'#7D8FA9',l:'Neutral'}
  return (
    <div>
      {NEWS.map((n,i)=>{
          ...
          <div key={i} style={{ display:'flex', gap:10, padding:'10px 14px', borderBottom:i<NEWS.length-1?'1px solid #F5F7FB':'none', cursor:'pointer' }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', gap:5, marginBottom:3, flexWrap:'wrap' }}>
                <span style={{ fontSize:9, padding:'1px 5px', borderRadius:999, background:'#F0F4FA', color:'#7D8FA9', fontWeight:600 }}>{n.tag}</span>
                <span style={{ fontSize:9, padding:'1px 5px', borderRadius:999, background:bg, color:c, fontWeight:700 }}>{l}</span>
                <span style={{ fontSize:9, color:'#B0BCD0', marginLeft:'auto' }}>{n.mins}m</span>
              </div>
              <p style={{ fontSize:12, fontWeight:700, color:'#0A1628', lineHeight:1.35, margin:0 }}>{n.headline}</p>
            </div>
          </div>
        )
      })}
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:3 }}>
                <span style={{ fontSize:11, fontWeight:700, color:'#0A1628' }}>{tr.co}</span>
                <span style={{ fontSize:9, padding:'1px 5px', borderRadius:999, background:'#F0F4FA', color:'#7D8FA9', fontWeight:600 }}>{tr.qtr}</span>
              </div>
              <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:4 }}>
                <span style={{ fontSize:10, fontWeight:600, color:'#3D4F6E' }}>{tr.speaker}</span>
                <span style={{ fontSize:10, color:'#B0BCD0' }}>· {tr.role}</span>
              </div>
...
const FILINGS = [
  ...
  { sym:'NVDA', co:'NVIDIA Corp.',  type:'8-K',  title:'Blackwell B300 Production Update',    mins:18, tag:'Product',  docColor:'#059669' },
        ...
        <div key={i} style={{ display:'flex', gap:10, padding:'10px 14px', borderBottom:i<FILINGS.length-1?'1px solid #F5F7FB':'none', cursor:'pointer' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ display:'flex', gap:5, marginBottom:3 }}>
              <span style={{ fontSize:9, padding:'1px 5px', borderRadius:999, background:'#EEF3FF', color:'#1B4FFF', fontWeight:700 }}>{f.type}</span>
              <span style={{ fontSize:9, padding:'1px 5px', borderRadius:999, background:'#F0F4FA', color:'#7D8FA9', fontWeight:600 }}>{f.tag}</span>
              <span style={{ fontSize:9, color:'#B0BCD0', marginLeft:'auto' }}>{f.mins}m</span>
            </div>
            <p style={{ fontSize:12, fontWeight:700, color:'#0A1628', lineHeight:1.35, margin:0 }}>{f.title}</p>
            <p style={{ fontSize:10, color:'#B0BCD0', marginTop:2 }}>{f.co}</p>
          </div>
        </div>
      ))}
    </div>
...
export function AiQueryWidget() {
  return (
    <div style={{ padding:'10px 14px', display:'flex', alignItems:'center', gap:10 }}>
      ...
      <Link href="/app/research" style={{ flex:1, fontSize:13.5, color:'#B0BCD0', textDecoration:'none' }}>Ask anything — earnings calls, filings, expert insights...</Link>
      <Link href="/app/research" style={{ background:'#1B4FFF', color:'#fff', border:'none', borderRadius:8, padding:'6px 14px', fontSize:12, fontWeight:700, textDecoration:'none', whiteSpace:'nowrap' }}>Open Research →</Link>
    </div>
  )
}
    ...
    <div style={{ padding:'12px 14px' }}>
      <div style={{ display:'flex', gap:6, marginBottom:10 }}>
        {['NVDA','TSLA','MSFT'].map(s=>(
          <button key={s} onClick={()=>setSym(s)} style={{ fontSize:11, fontWeight:700, padding:'3px 8px', borderRadius:6, border:'none', cursor:'pointer', fontFamily:'inherit', background:sym===s?cBg(s):'#F0F4FA', color:sym===s?'#fff':'#7D8FA9' }}>{s}</button>
        ))}
      </div>
      <div style={{ display:'flex', gap:8, marginBottom:8 }}>
        ...
        <div>
          <div style={{ fontSize:12, fontWeight:700, color:'#0A1628' }}>{sym}</div>
          <div style={{ fontSize:10, color:'#059669', fontWeight:600 }}>AI Summary</div>
        </div>
      </div>
      <p style={{ fontSize:12.5, color:'#1C2B4A', lineHeight:1.6, margin:0 }}>{summaries[sym]}</p>
      <Link href={`/app/company/${sym}`} style={{ fontSize:11, fontWeight:700, color:'#1B4FFF', textDecoration:'none', marginTop:8, display:'inline-block' }}>View full profile →</Link>
    </div>
  )
}
    ...
    <div>
      {WL.map((q,i)=>(
        <div key={q.symbol} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 14px', borderBottom:i<WL.length-1?'1px solid #F5F7FB':'none', cursor:'pointer' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:12, fontWeight:700, color:'#0A1628' }}>{q.symbol}</div>
            <div style={{ fontSize:10, color:'#B0BCD0', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{q.name}</div>
          </div>
          <div style={{ textAlign:'right' }}>
            <div style={{ fontSize:12, fontWeight:800, color:'#0A1628' }}>${q.price.toFixed(2)}</div>
            <div style={{ fontSize:11, fontWeight:700, color:q.chg>=0?'#059669':'#DC2626' }}>{q.chg>=0?'+':''}{q.chg.toFixed(2)}%</div>
          </div>
        </div>
      ))}
    </div>
