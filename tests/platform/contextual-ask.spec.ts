...
test.describe("contextual ask surface", () => {
  ...
  for (const { path, label } of PAGES_WITH_ASK_BAR) {
    test(`ContextualAskBar mounts on ${label}`, async ({ page }) => {
      ...
      // The company page calls setLoading(false) only after 13 parallel API
      // requests AND a sequential chart-data fetch complete; under full-suite
      // load those can take up to ~20 s. A 90 s assertion window stays safely
      // within the 120 s test budget even after a slow navigation.
      test.setTimeout(120_000);
      await gotoWithRetry(page, path);
      const ask = page.getByRole("textbox", { name: /^Ask Finsyt about/i }).first();
      await expect(ask).toBeVisible({ timeout: 90_000 });
    });
  }

  test("global shortcuts: '/' focuses Ask input and ⌘J opens drawer", async ({ page }) => {
    test.setTimeout(120_000);
    await gotoWithRetry(page, "/platform/app");
    ...
    // Click an inert area first so focus starts outside the input, then press
    // "/" to focus the CommandInput. Under full-suite load the global keydown
    // handler can attach a beat after the input paints, so poll the
    // blur → "/" → focus cycle until it lands instead of flaking on a race.
    await expect(async () => {
      await page.locator("body").click({ position: { x: 4, y: 4 } });
      await page.keyboard.press("/");
      await expect(askInput).toBeFocused({ timeout: 1_000 });
    }).toPass({ timeout: 15_000 });

    // ⌘J / Ctrl+J opens the Ask AI drawer. Poll in the same style as the
    // "/" focus cycle above so a timing/focus race (the handler attaches
    // after the first paint) never kills the test under full-suite load.
    const drawer = page.getByRole("dialog", { name: /Finsyt Agent|Ask Finsyt|Finsyt Copilot|Ask drawer/i }).first();
    await expect(async () => {
