...
test.describe("recent decks history panel", () => {
  ...
  test("company page exposes a Recent decks drawer with re-download rows", async ({
    ...
    // The company page starts in a loading skeleton (loading=true) while 13
    // parallel API calls resolve. The "Export to pitch deck" button is gated
    // behind that guard — it only renders after loading=false AND quote.price
    // is set. Under full-suite load with rate-limited upstream providers each
    // call can take 3–5 s, so 60 s gives comfortable headroom.
