...
test.describe("peers workspace + copilot wiring", () => {
  ...
  test("/app/peers renders the Peers workspace with seeded sets", async ({ page }) => {
    ...
    const resp = await gotoWithRetry(page, "/platform/app/peers");
  ...
  test("/app/company/AAPL/peers renders the Selected Peers table", async ({ page }) => {
    test.setTimeout(120_000);
    const resp = await gotoWithRetry(page, "/platform/app/company/AAPL/peers");
    expect(resp?.status()).toBe(200);
  ...
  test("/peers slash command in the topbar navigates to /app/peers", async ({ page }) => {
    test.setTimeout(120_000);
    // The first-run welcome modal intercepts pointer events until dismissed.
    // Pre-seed the localStorage flag so it does not block clicks on the Ask
    // input (same pattern as the matrix and recent-decks smoke tests).
    await page.addInitScript(() => {
      try {
        window.localStorage.setItem("finsyt:firstrun:done", String(Date.now()));
      } catch {
