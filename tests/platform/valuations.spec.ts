...
test.describe("football field valuations surface", () => {
  ...
  test("picker page mounts at /platform/app/valuations", async ({ page }) => {
    ...
    await gotoWithRetry(page, "/platform/app/valuations");
  ...
  test("chart page renders Football Field for /AAPL", async ({ page }) => {
    test.setTimeout(120_000);
    await gotoWithRetry(page, "/platform/app/valuations/AAPL");
    ...
    // fabricated band. Possible captions: "Loading precedent transactions…",
    // "No M&A deal data available", "No comparable transactions found", or
    // "N precedent transactions · FMP" when real data is present.
    // We check for the Transaction Comps group header (always rendered) plus
    // at least one Implied EV row label to confirm the section is present.
    await expect(page.getByText(/Implied EV \/ Revenue/i).first()).toBeVisible();
  ...
  test("invalid ticker shows the no-quote empty state, not a fabricated chart", async ({ page }) => {
    test.setTimeout(120_000);
    await gotoWithRetry(page, "/platform/app/valuations/INVALID_TICKER_XYZ");
  ...
  test("Valuations tab on company page mounts the same chart", async ({ page }) => {
    test.setTimeout(120_000);
    await gotoWithRetry(page, "/platform/app/company/AAPL?tab=valuations");
    await expect(
      page.getByText(/Valuation Overview\s*·\s*AAPL/i).first(),
    ).toBeVisible({ timeout: 45_000 });
  });
