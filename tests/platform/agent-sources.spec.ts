...
test.describe("agent sources, citations, drawer and follow-ups", () => {
  ...
  test("research page shows sources panel, inline citation, drawer, and follow-ups", async ({
    ...
    // The research page has multiple `aside[role="dialog"]` elements: the
    // "Create Alert" and "Source Library" panels lack aria-label, while the
    // source-detail drawer gets `aria-label="Source N: <label>"`. Scoping to
    // the aria-label variant avoids a Playwright strict-mode violation.
    const drawer = page.locator('aside[role="dialog"][aria-label]');
