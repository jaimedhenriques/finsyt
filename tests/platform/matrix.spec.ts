...
/**
 * Navigate to the matrix workspace, retrying on the dev-only cold-compile
 * `net::ERR_ABORTED`. On first visit to a heavy route, the Next.js dev server
 * triggers a reload once the RSC payload is ready, which can abort the in-flight
 * navigation. Retrying lets the warmed-up compile pass succeed instead of
 * flaking the suite. Returns the final navigation response.
 *
 * Uses 5 attempts × 20 s each (100 s max) so even two aborted compiles
 * leave 20 s of headroom inside the 120 s test budget.
 */
async function gotoMatrix(page: Page, query = "") {
  let lastErr: unknown = null;
  const path = `/platform/app/matrix${query}`;
  ...
  throw lastErr ?? new Error("gotoMatrix: navigation failed");
}
...
test.describe("matrix workspace", () => {
  ...
  test("/app/matrix mounts with the three default entity rows", async ({ page }) => {
    ...
    const resp = await gotoMatrix(page);
  ...
  test("'+ Add column' prompts append a new question column", async ({ page }) => {
    ...
    // The new column header renders in the grid <thead>. The append re-renders
    // the whole grid, which can lag well past the default expect window when
    // the dev server is mid-compile under full-suite load — give it room.
  ...
  test("Snapshots toolbar button opens the SnapshotsDrawer", async ({ page, baseURL }) => {
    ...
    await gotoMatrix(page, `?id=${matrixId}`);
