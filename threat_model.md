...
- **Highest-risk code areas:** unauthenticated Next.js API routes that call LLMs, token relays, or external services; workspace ingest/chat/studio routes; admin proxy routes using `artifacts/platform/lib/audit-client.ts`; Express auth/actor middlewares; tenant-scoped DB wrappers and RLS.
- **Public surfaces:** marketing site, `POST /api/leads`, legacy `/api/mcp`, helper routes such as `/api/an-token`, any Next.js API route omitted from explicit auth checks (including paid-provider helpers like `/api/macro` and `/api/transcripts`), and any other Next.js API route without explicit auth. `/api/v1/**` is the intended public API surface and should require API-key auth.
